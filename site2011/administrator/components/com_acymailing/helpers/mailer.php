...
?>
<?php
require_once(ACYMAILING_BACK.'inc'.DS.'phpmailer'.DS.'class.phpmailer.php');
class mailerHelper extends acymailingPHPMailer {
	...
	function mailerHelper() {
		...
		switch ( $this->config->get('mailer_method','phpmail') )
		{
			case 'smtp' :
				...
				if(empty($this->Sender)) $this->Sender = $this->Username;
				break;
			case 'sendmail' :
				$this->IsSendmail();
		...
		if(empty($this->Encoding)) $this->Encoding = 'base64';
	...
	function clearAll(){
		...
		$this->ClearReplyTos();
		$this->errorNumber = 0;
		$this->addCustomHeader( 'X-Mailer: ' . preg_replace('#https?://#','',trim(ACYMAILING_LIVE,'/')));
	}
	function sendOne($mailid,$receiverid){
		$this->clearAll();
		$this->addCustomHeader( 'X-Mailid: ' . $mailid );
		...
		if(!isset($this->forceVersion)){
			if($this->checkConfirmField AND empty($receiver->confirmed) AND $this->config->get('require_confirmation',0)){
		...
		if(!empty($this->parameters)){
			...
			if(!empty($this->AltBody)) str_replace($keysparams,$this->parameters,$this->AltBody);
		}
		if($this->sendHTML){
	...
	function embedImages(){
	    ...
	    if(!empty($images[2])) {
	      ...
	      foreach($images[2] as $i => $url) {
	      	...
	      	$allimages[$url] = 1;
	      	$path = str_replace(array(ACYMAILING_LIVE,'/'),array(ACYMAILING_ROOT,DS),$url);
	...
	function textVersion($html,$fullConvert = true){
		...
		$replaceByNothing = '#< */ *(p|li|td|tr)[^>]*>#Ui';
		$replaceByReturnChar = '#< */? *(br|p|h1|h2|h3|li|ul|h4|h5|h6|tr|td)[^>]*>#Ui';
		$replaceLinks = '/< *a[^>]*href *= *"([^"]*)"[^>]*>(.*)< *\/ *a *>/Uis';
		$text = preg_replace(array($replaceByTwoReturnChar,$replaceByStars,$replaceByNothing,$replaceByReturnChar,$replaceLinks),array("\n\n","\n* ",'',"\n",'${2} ( ${1} )'),$html);
