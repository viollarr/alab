...
<?php
		echo JText::_('FIELD_TEXT').' : <input name="tagtext" size="100px" onchange="setOnlineTag();"><br/><br/>';
		$radios = array();
		...
		echo JText::_('DISPLAY').JHTML::_('select.radiolist', $radios, 'template' , 'size="1" onclick="setOnlineTag();"', 'value', 'text', 'notemplate');
	...
	function acymailing_replaceusertags(&$email,&$user,$send = true){
		...
		foreach($results as $var => $allresults){
			foreach($allresults[0] as $i => $oneTag){
				...
				$tag->template = ($tag->type == 'readonline') ? $this->params->get('viewtemplate','standard') : $this->params->get('forwardtemplate','standard');
