...
class plgAcymailingTagtime extends JPlugin
{
	function plgAcymailingTagtime(&$subject, $config){
	 ...
	 function acymailingtagtime_show(){
		...
		foreach($others as $tagname => $tag){
			$text .= '<tr style="cursor:pointer" class="row'.$k.'" onclick="setTag(\''.$tagname.'\');insertTag();" ><td>'.$tag.'</td><td>'.acymailing_getDate(time(),JText::_($tag)).'</td></tr>';
			$k = 1-$k;
		}
		$text .= '</table>';
		echo $text;
	...
	function acymailing_replacetags(&$email,$send = true){
		...
		foreach($variables as $var){
			$email->$var = str_replace(array('{mailid}','%7Bmailid%7D'),array($email->mailid,$email->mailid),$email->$var);
		}
		$found = false;
		foreach($variables as $var){
