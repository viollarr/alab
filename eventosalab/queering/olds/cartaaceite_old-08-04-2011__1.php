<?php
	...
	$sql_simposio = "SELECT s.id, s.titulo_sessao
	...
	$sql_debatedor_simposio = "SELECT s.id, s.titulo_sessao
							  FROM ev_simposio s
							  WHERE  
	...
	$sql_coordenada = "SELECT cc.id, cc.titulo_sessao
	...
	$sql_comunicacao_individual = "SELECT r.id
								  ...
								  ORDER BY r.titulo ASC";
	$comunicacao_individual = mysql_query($sql_comunicacao_individual, $conexao) or die(mysql_error());
	$existe_comunicacao_individual = mysql_num_rows($comunicacao_individual);
         ...
         <div id="box_trabalhos"><span class="txt_titulo_destaque">Carta de Aceite</span>
           ...
           <?php if ($existe_simposio>0){
					while($mostrar_simposio= mysql_fetch_array($simposio)){ ?>
						<p class="destaque"><span class="txt_categorias"><strong>Simp&oacute;sio</strong></span></p>
						<p class="destaque"><?php print $mostrar_simposio['titulo_sessao'];?></p>
						<span class="menuinterno"><a href="gerar_carta_aceite.php">Baixar Carta</a></span>
                        <p>&nbsp;</p>
                        <?php 
					} //while
			} //if 
