	...
	*/
	...
	if ($_POST["insert"] == "true"){
		...
		if ($id_trabalho==1){ //print "simposio";
			...
			$ids_formacao_autor = array($id_formacao_autor0, $id_formacao_autor1, $id_formacao_autor2, $id_formacao_autor3);
			...
			$ids_formacao_co_autor = array($id_formacao_co_autor0, $id_formacao_co_autor1, $id_formacao_co_autor2, $id_formacao_co_autor3);
			...
			for($i = 0; $i <= 3; $i++){
				...
				if( !empty($titulos[$i]) ){
					if(empty($autores[$i])) {$error[]="Preencha o nome do autor no Trabalho $k."; }
					if(empty($emails_autor[$i])) {$error[]="Preencha o email do autor <b>$autores[$i]</b> no Trabalho $k."; }
					if(empty($ids_formacao_autor[$i])) {$error[]="Preencha a formação do autor <b>$autores[$i]</b> no Trabalho $k."; }

					if(!empty($coautores[$i])) {
						if(empty($emails_coautor[$i])) {$error[]="Preencha o email do co-autor <b>$coautores[$i]</b> no Trabalho $k."; }
						if(empty($ids_formacao_co_autor[$i])) {$error[]="Preencha a formação do co-autor <b>$coautores[$i]</b> no Trabalho $k."; }
					}//if
					
					if(empty($resumos[$i])) {$error[]="Preencha o resumo do Trabalho $k."; }
					if(empty($palavras[$i])) {$error[]="Preencha as palavras-chave do Trabalho $k."; }
				}//if
			}//foreach
			...
			if(sizeof($error)==0){
				...
				for($i=0; $i<$qtde_trab; $i++){		
					if (!empty($array_trabalhos[$i])){
						...
						$email_autor=trim($_POST['email_autor'][$i]);
						$coautor=addslashes($_POST['coautor'][$i]);
						$email_coautor=trim($_POST['email_coautor'][$i]);					
							...
							if ($id_autor!=$id_participante){
								...
								if ($email_ja_cadastrado>0){//manda e-mail informando que a pessoa foi inserido no simposio						
									...
									$sql_insert = "insert into ev_participante(id_evento,nome,email,senha,presenca)
												   values('$id_evento','$autor','$email_autor','$senha_random','nao');";	
									mysql_query($sql_insert, $conexao);	
									$id_autor=mysql_insert_id();	
