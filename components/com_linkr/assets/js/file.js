...
var LinkrFile	=
{
	...
	setDirectoryList : function(i)
	{
		...
		else if ($type(this.directoryList) === false)
		{
			...
			/*return new Json.Remote(u, {
				onComplete : LinkrFile.setDirectoryList.bind(this)
			}).send();*/
		}
	...
	_ufsc : function(d) {
		...
		/*new Json.Remote(u, {
			onComplete : LinkrFile._files.bind(this)
		}).send();*/
	},
	_files : function(i)
	{
	...
	_simpleFileList : function(div, i)
	{
		...
		if (i.files.length > 0)
		{
			...
			i.files.each(function(f)
			{
				...
				if (ix.contains(ext)) {
					//_n.clone().setHTML('&bull;').injectInside(item);
	...
	_normalFileList : function(div, i)
	{
		...
		var _b	= new Element('div', {'class' : 'icon'});
		...
		if (i.folders.length > 0)
		{
			...
			i.folders.each(function(f)
			{
				...
				var border	= _c.clone().injectInside(icon);
				var link	= _d.clone().injectInside(border);
				var img		= fimg.clone().injectInside(link);
				var name	= _f.clone().setHTML(f.name).injectInside(item);
		...
		if (i.files.length > 0)
		{
			...
			i.files.each(function(f)
			{
				...
				icon.setProperty('title', f.type +' - '+ f.name +' ('+ f.size +')');
