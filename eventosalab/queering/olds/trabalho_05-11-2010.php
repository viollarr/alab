<?php
	...
	*/
				...
				function envia_email_recebimento($tipo_trab, $titulo, $email_para_quem){
					$nome_evento  = "";
					$dia_evento   = "";
					$local_evento = "";
					...
					$quemenvia="ALAB";
					$email_alab = "alab@alab.org.br";
					$assunto = "Confirmação de recebimento de proposta de trabalho";
							
					$corpoemail = "Confirmamos o recebimento da sua proposta de (comunicação coordenada/comunicação individual/pÿster) intitulada (nome da proposta inserido pelo participante) para apresentação no ".$nome_evento.", a ser realizado no período de ".$dia_evento." em ".$local_evento;
		...
		if ($id_trabalho==4){
			...
			if(sizeof($error)==0){
				...
				if($registro_inserido){
					exit("Pôster inserido");
					//envia_email_recebimento($tipo_trab, $titulo, $email_para_quem);
				}
			}
		}
