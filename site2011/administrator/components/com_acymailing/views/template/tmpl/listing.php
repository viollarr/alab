...
<form action="index.php?option=<?php echo ACYMAILING_COMPONENT ?>&amp;gtask=template" method="post" name="adminForm">
	<table>
		<tr>
			<td width="100%">
		...
		<thead>
			<tr>
				...
				<th class="title titletoggle">
					<?php echo JHTML::_('grid.sort',   JText::_( 'DEFAULT'), 'a.premium', $this->pageInfo->filter->order->dir, $this->pageInfo->filter->order->value ); ?>
				</th>
				<th class="title titletoggle">
				...
				<th class="title titleid">
					<?php echo JHTML::_('grid.sort',   JText::_( 'ID' ), 'a.tempid', $this->pageInfo->filter->order->dir, $this->pageInfo->filter->order->value ); ?>
				</th>
			</tr>
		</thead>
		<tfoot>
		...
		<tbody>
			...
			?>
				<tr class="<?php echo "row$k"; ?>">
					...
					<td>
						<a href="<?php echo acymailing::completeLink('template&task=edit&cid[]='.$row->tempid); ?>"><?php echo $row->name; ?></a>
					</td>
					<td>
						<?php echo acymailing::absoluteURL($row->description); ?>
					</td>
					<td class="order">
