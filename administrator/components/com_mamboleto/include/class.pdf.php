...
var $infoObject=0;
var $numImages=0;
var $options=array('compression'=>0);
  ...
  switch ($action){
    ...
    case 'out':
      if (count($this->objects[$id]['info']['pages'])){
        ...
        if (count($this->objects[$id]['info']['fonts']) || isset($this->objects[$id]['info']['procset'])){
          $res.="\n/Resources <<";
          if (isset($this->objects[$id]['info']['procset'])){
            $res.="\n/ProcSet ".$this->objects[$id]['info']['procset']." 0 R";
          }
          if (count($this->objects[$id]['info']['fonts'])){
            $res.="\n/Font << ";
          ...
          if (count($this->objects[$id]['info']['xObjects'])){
            $res.="\n/XObject << ";
...
function o_outlines($id,$action,$options=''){
  switch ($action){
    case 'new':
      $this->objects[$id]=array('t'=>'outlines','info'=>array());
...
//  $this->o_pages(3,'mediaBox',array(0,0,612,792));
...
//  $this->objects[7]=array('t'=>'content','c'=>'');
//  $this->numObj=7;
}
