 ...
 */
(function($){$.WFBrowserWidget={options:{element:null,plugin:{plugin:'browser',root:'',site:'',help:function(){var win=window.parent;if(typeof win.$jce!=='undefined'){window.parent.$jce.createDialog({src:'index.php?option=com_jce&view=help&tmpl=component&section=editor&category=browser',type:'help',options:{width:780,height:560}});}else{$.Dialog.iframe('Help','index.php?option=com_jce&view=help&tmpl=component&section=editor&category=browser',{width:780,height:560});}}},manager:{upload:{insert:false},expandable:false}},init:function(options){var self=this,win=window.parent,doc=win.document;$.extend(true,this.options,options);$('<input type="hidden" id="src" value="" />').appendTo(document.body);$.Plugin.init(this.options.plugin);$('button#insert, button#cancel').hide();if(this.options.element){$('button#insert').show().click(function(e){self.insert();self.close();e.preventDefault();});$('button#cancel').show().click(function(e){self.close();e.preventDefault();});var src=doc.getElementById(this.options.element).value||'';$('#src').val(src);}
...
if(typeof win.SqueezeBox!=='undefined'){return win.SqueezeBox.close();}}};})(jQuery);var tinyMCE={addI18n:function(p,o){return $.Plugin.addI18n(p,o);}};