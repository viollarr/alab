		...
		foreach($lists as $status => $listids){
			...
			foreach($listids as $listid){
				...
				if($status > 0 && acymailing::level(3)){
					if(!$app->isAdmin() && $allResults[$listid]->access_sub != 'all'){
						if($allResults[$listid]->access_sub == 'none' OR empty($my->id) OR empty($my->gid)) continue;
						if(!in_array($my->gid,explode(',',$allResults[$listid]->access_sub))) continue;
					}
				}
