...
class PhocaFaviconCpModelPhocaFaviconi extends JModel
{
		...
		if ($file_list !== false)
		{
			foreach ($file_list as $file)
			{
				if (is_file($orig_path.DS.$file) && substr($file, 0, 1) != '.' && strtolower($file) !== 'index.html')
				{
					...
					$refresh_url 	= 'index.php?option=com_phocafavicon&view=phocafaviconi&tmpl=component&folder='.$current;
					$file_no		= $current . "/" . $file;
					$file_thumb 	= PhocaFaviconHelper::getOrCreateThumbnail($path['orig_abs_ds'], $file_no, $refresh_url, 0, 1, 0);
