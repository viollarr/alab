...
if (isset( $_POST['submit'] )) {
	...
	if (@$my->id) {
		...
		if(isset($grp->jaclplus) && class_exists('JACLPlus')) $my->jaclplus	= $grp->jaclplus;
		...
		. ( isset($my->jaclplus) ? ", gid = " . (int) $my->gid . ", jaclplus = " . $database->Quote( $my->jaclplus ) : "" )
		;
		$database->setQuery( $query );
		if (!$database->query()) {
		...
		if(isset($my->jaclplus)) $_SESSION['session_jaclplus'] 	= $my->jaclplus;
