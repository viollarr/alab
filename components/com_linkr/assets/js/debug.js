...
var LinkrDebug	=
{
	...
	{
		this.s	= {
			img : Linkr.siteRoot +'components/com_linkr/assets/img/'
		};
	...
	home : function()
	{
		var h	= 'Use this page to test Linkr\'s basic functions.<br /><br />';
		h	+= this._plk('Database Tests', 'db');
		h	+= this._plk('Files &amp; Folders Tests', 'ff');
		Linkr.htmlLayout('Linkr Debug', this.hd(h));
	},
	_plk : function(t, c) {
	...
	ff : function()
	{
		...
		c.include(this.hd('Files &amp; Folders'));
		c.include(this.td('ffd', 'Directory list'));
		c.include(this.td('fff', 'Get files in this directory:', 'images/stories'));
	...
	fff : function(r)
	{
		if (r !== 1) return this.udb('fff', r);
		
		this.itd('fff');
		var f	= $('fff-i').getValue().replace(/[^0-9a-z_\.\-\/]/ig, '');
		if (f.length < 1) f = 'images/stories';
		$('fff-i').value	= f;
		Linkr.listFiles(f, 'all', this.fff.bind(this));
	},
	...
	// Inserts loading icon in test DIV
	itd : function (id) {
