...
*/
/**
* @version $Id: gacl.class.php 87 2005-09-15 23:12:03Z eddieajau $
* @package Joomla
...
class gacl {
	...
	function gacl( $db=null ) {
		...
		if(class_exists('JACLPlus')) {
			$this->CacheLite =& mosCache::getCache( 'com_jaclplus' );
		}else {
		// backend login
	...
	*/
	function _mos_add_acl( $aco_section_value, $aco_value,
		...
		if(class_exists('JACLPlus')) $this->_create_acl( $aro_section_value, $aro_value );
	...
	\*======================================================================*/
	/* Hack for integration with JACLPlus Component */
			...
			if (strcasecmp( $aco_section_value, $this->acl[$i][0] ) == 0) {
				if (strcasecmp( $aco_value, $this->acl[$i][1] ) == 0) {
					if (strcasecmp( $aro_section_value, $this->acl[$i][2] ) == 0) {
						if (strcasecmp( strtolower($aro_value), $this->acl[$i][3] ) == 0) {
	...
	/* End of hack for JACLPlus Component */
	
	function _create_acl( $aro_section_value, $aro_value ){
		if(!defined('_JACLPLUS_')) {
			define('_JACLPLUS_', 1);
			global $my;
			
			$aro_value = $my->usertype;
			$aro_section_value = "users";
			$cache_id = 'acl_query_'.$aro_section_value.'-'.$aro_value;
			
			$retarr = FALSE;
			if ( is_string($this->CacheLite->_cache->get($cache_id, $this->CacheLite->_cache->_defaultGroup) ) ) {
				$retarr = unserialize($this->CacheLite->_cache->get($cache_id, $this->CacheLite->_cache->_defaultGroup) );
			}
	
			if ($retarr) {
				$this->acl = $retarr;
			} else {
				$query = "SELECT * FROM #__jaclplus WHERE "
					."aro_section ='".$this->db->getEscaped($aro_section_value)."' AND "
					."aro_value ='".( $aro_value ? $this->db->getEscaped(strtolower($aro_value)) : "public frontend")."' AND "
					."enable=1 ";
				$this->db->setQuery( $query );
				$rows = $this->db->loadObjectList();
				if(count($rows)>0) {
					foreach ($rows as $row) {
						if(empty($row->axo_section)){
							//$this->_mos_add_acl( $row->aco_section, $row->aco_value, $row->aro_section, $row->aro_value, NULL, NULL );
							$this->acl[] = array( $row->aco_section, $row->aco_value, $aro_section_value, strtolower($aro_value), NULL, NULL );
						} else {
							//$this->_mos_add_acl( $row->aco_section, $row->aco_value, $row->aro_section, $row->aro_value, $row->axo_section, $row->axo_value );
							$this->acl[] = array( $row->aco_section, $row->aco_value, $aro_section_value, strtolower($aro_value), $row->axo_section, $row->axo_value );
						}
					}
				}
				// prevent return empty array to avoid query multiple times
				if(empty($this->acl)) $this->acl[] = array( 'aco_section', 'aco_value', 'aro_section', 'aro_value', 'axo_section', 'axo_value' );
				//Cache data.
				$this->CacheLite->_cache->save(serialize($this->acl), $cache_id, $this->CacheLite->_cache->_defaultGroup);
			}
			$this->acl_count = count( $this->acl );
		}
	}
