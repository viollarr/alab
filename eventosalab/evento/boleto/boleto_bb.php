...
// DADOS DO BOLETO PARA O SEU CLIENTE
	...
	$sql_participante = "SELECT ev.id, us.name, us.endereco_res, us.cep_res, us.tipo_residencia, us.id_estado_res, us.id_cidade_res, us.estado_res, us.cidade_res FROM ev_participante ev, jos_users us WHERE ev.id='".$id_participante."' AND ev.id_associado_alab = us.id";
...
if($p['tipo_residencia'] == 1){
	$select_estado_cidade	= "SELECT es.sigla, ci.nome FROM ev_estados es, ev_cidades ci WHERE ci.cod_cidades = '".$p['id_cidade_res']."' AND ci.estados_cod_estados = es.cod_estados LIMIT 1";
	$query_estado_cidade	= mysql_query($select_estado_cidade);
	$res_estado_cidade		= mysql_fetch_array($query_estado_cidade);
	$estado = $res_estado_cidade['sigla'];
	$cidade = $res_estado_cidade['nome'];
}
else{
	$estado = $p['estado_res'];
	$cidade = $p['cidade_res'];
...
$dadosboleto["sacado"] = $p['name'];
$dadosboleto["endereco1"] = $p['endereco_res']." - ".$cidade." - ".$estado ;
$dadosboleto["endereco2"] = 'CEP: '.$p['cep_res'];
