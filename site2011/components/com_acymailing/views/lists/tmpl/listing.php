	...
	foreach($this->rows as $i => $oneList){
	    ...
	    if(acymailing::level(3)){
	    	...
	    	if(!empty($my->gid) AND !empty($my->id)){
	    		if($row->access_manage == 'all' OR in_array($my->gid,explode(',',$row->access_manage))){
	    			 $frontEndManagement = true;
	    		}
	    	}
	    	if($row->access_sub != 'all' AND ($row->access_sub == 'none' OR empty($my->gid) OR empty($my->id) OR !in_array($my->gid,explode(',',$row->access_sub)))){
