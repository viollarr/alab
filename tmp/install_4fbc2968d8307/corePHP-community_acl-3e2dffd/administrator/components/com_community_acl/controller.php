...
class UsersController extends JController
{
	...
	function save()
	{
		...
		$query = "INSERT INTO `#__community_acl_user_params` "
					." SET `user_id` = '".$user->get('id')."', "
						." `name` = 'publisher_notification', "
						." `value` = '". $publisher_notification ."' ";
		$db->setQuery( $query );
		$db->query();
			...
			else
			{
				...
				else
				{
					...
					if ($sid > 0) {
						...
						if ($config->synchronize && $config->users_and_cb) {
							$sync = new CACL_syncronize($main);
							$sync->syncronize($user->get('id'), 'user_delete');
							$sync->syncronize($user->get('id'), 'cb_user_delete'); 
						}
					}
					// delete user
					$user->delete();
	...
	function cancel( )
	{
		$this->setRedirect( 'index.php?option=com_community_acl&mode=manage_users' );
	}
	...
	function logout( )
	{
		...
		if ( count( $cids ) < 1 ) {
			$this->setRedirect( 'index.php?option=com_community_acl&mode=manage_users', JText::_( 'User Deleted' ) );
			return false;
		}
