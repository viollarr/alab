...
class plgContentLinkr_content extends JPlugin
{
	...
	function plgContentLinkr_content(&$subject, $config)
	{
		...
		$this->loadLanguage('plg_content_linkr_content', JPATH_ADMINISTRATOR);
		
		// Fix params (MyBlog disables these... why?)
		...
		$this->linkrParams	= & JComponentHelper::getParams('com_linkr');
	}
	
	/*
	 * Plugin tools
	 *
	 * @param float $version	Linkr Version
	 * @return array		Array of tools for plugin
	 */
	function onLinkrGetTools($version)
	{
		...
		if (LinkrHelper::getParam('debug', 0) && $user->gid >= 23) {
			$tools['Debug']	= 'LinkrDebug.load();';
		}
	 ...
	 *
	 * @param float $version	Linkr Version
	 * @return array		Array of links for plugin
	 */
	function onLinkrGetLinks($version)
	{
		...
		// Files
		if ($this->params->get('link_file', 1))
		{
			$links[]	= array(
				'name'	=> 'file',
				'text'	=> JText::_('FILE'),
		...
		// Articles
		if ($this->params->get('link_article', 1))
		{
			$links[]	= array(
				'name'	=> 'article',
		...
		// Menu links
		if ($this->params->get('link_menu', 1))
		{
			$links[]	= array(
				'name'	=> 'menu',
				'text'	=> JText::_('MENU'),
		...
		// Contacts
		if ($this->params->get('link_contact', 1))
		{
			$links[]	= array(
				'name'	=> 'contact',
	 ...
	 *
	 * @param float $version	Linkr Version
	 * @param array $inc		Array of all included and excluded links
	 * @return string		Javascript statement
	 */
	function onLinkrLoadJS($version, $inc = array())
	{
		// File append
		$append	= $this->params->get('compress', 1) ? '.js' : '-UCP.js';
		...
		if ($inc['bookmarks'])
		{
			...
			$bm	= LinkrHelper::getPluginParam('def_bm', 0) ? 'true' : 'false';
		...
		if ($inc['related'])
		{
			...
			$rl	= LinkrHelper::getPluginParam('def_rel', 0) ? 'true' : 'false';
			$js	.= 'LinkrRelated.loadedByDefault='. $rl .';'.
					'LinkrRelated.isPluginInstalled='. $p .';';
		}
		
		// Files
		if ($this->params->get('link_file', 1) && $inc['file'])
		{
			$d->addScript($r .'file'. $append);
			$u	= (bool) LinkrHelper::getParam('frontend_upload', 0);
		...
		// Articles
		if ($this->params->get('link_article', 1) && $inc['article'])
		{
			$d->addScript($r .'article'. $append);
			$s	= (bool) LinkrHelper::getPluginParam('use_slug', 1);
		...
		// Menu links
		if ($this->params->get('link_menu', 1) && $inc['menu'])
		{
		...
		// Contacts
		if ($this->params->get('link_contact', 1) && $inc['contact'])
		{
	 ...
	 *
	 * @param float $version	Linkr Version
	 * @return array		Array of strings to localize and UTF8 encode to client side
	 */
	 ...
	 * Linkr magic
	 *
	 *	NOTE: Other extensions besides Joomla! trigger this event...
	 *
	 * @param object $article	The article object (by reference)
	 * @param object $params	The article parameters object (by reference)
	 * @param int $limitstart	Article index when article is separated
	 * @return void
	 */
	function onPrepareContent(&$article, &$params, $limitstart = 0)
	{
		...
		if ($fm != 'html' || $tm != 'index' || JRequest::getBool('print', false)) {
			...
			$this->showBookmarks	= ($this->params->get('bm_fp', 0));
			$this->showRelated		= ($this->params->get('rel_fp', 0));
		}
	...
	function getBookmarks( $sets )
	{
		...
		$tips	= (bool) $this->params->get('bm_tips', 1);
		$tips ? JHTML::_('behavior.tooltip') : null;
		$_size	= $sets->get('size', $this->params->get('bm_size', 'text'));
		$html	= 'html'. $_size;
		$content	= array();
		foreach ($list as $bm)
		{
		...
		$sep	= JString::strlen($sep) ? $sep : $this->params->get('bm_sep', ' ');
	 ...
	 * @param string $html	HTML code for bookmark (by reference)
	...
	function getRelatedLinks( $sets )
	{
		...
		$limit		= $sets->get('limit', $this->params->get('rel_limit', 5));
		...
		if ($this->params->get('rel_lsec', 1)) {
			$where	.= ' AND a.sectionid = '. $this->article->sectionid;
		} else {
			$relx1	= (int) $this->params->get('rel_x1', 0);
			$relx2	= (int) $this->params->get('rel_x2', 0);
			$relx3	= (int) $this->params->get('rel_x3', 0);
		...
		// Order
		switch ($this->params->get('rel_sort', 'random'))
		{
			case 'title':
		...
		// Related links
		$text		= $sets->get('title', $this->params->get('rel_title', JText::_('REL_TITLE')));
		...
		// Default styles
		if ($this->params->get('rel_def_css', 1))
		{
			if (!isset($this->relStyles))
			{
				$this->relStyles	= true;
				$d	= & JFactory::getDocument();
				$s	= $this->linkrParams->get('rcss', '');
				$d->addStyleDeclaration(base64_decode($s));
			}
		}
	}
	...
	/**
	 * Creates array of keywords
	 *
	 * @return array	Article keywords
	 */
	function getKeywords($words = null)
	{
		...
		// Get keywords
		$source	= $this->params->get('key_source', 'article');
		switch ($source)
		{
			case 'article':
				$words	= $this->_akeys();
	 ...
	 *
	 * @param interger $off	Current position in text
	 * @return array		Linkr settings or false if none found
	 */
	...
	function getDefaultSettings()
	{
		...
		if ($this->showBookmarks && ! @$this->loaded['bookmarks'] && $this->params->get('def_bm', 0))
		{
			$q	= 'SELECT id FROM #__linkr_bookmarks';
			if (!$this->params->get('bm_select', 0)) {
				$q	.= ' WHERE popular = 1';
			}
			...
			if ($badges = $db->loadResultArray())
			{
				...
				$s->set('size', $this->params->get('bm_size', 'text'));
				$s->set('separator', $this->params->get('bm_sep', ' '));
		...
		if ($this->showRelated && ! @$this->loaded['related'] && $this->params->get('def_rel', 0))
		{
			...
			if (!empty( $words ))
			{
				...
				$s->set('limit', $this->params->get('rel_limit', 5));
	 ...
	 * Finds article route
	 *
	 * @param $a		Article ID or object
	 * @param bool $full	Set to "TRUE" to include site root
	...
	/*
	 * Determines if a string is UTF8-encoded
	 */
	function isUTF8($str)
	{
		if (is_array($str)) {
