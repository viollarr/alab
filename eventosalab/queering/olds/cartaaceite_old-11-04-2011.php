<?php
	...
	$sql_resumo_simposio = "SELECT r.id, r.titulo
							FROM ev_resumo r
							WHERE  
	...
	$resumo_simposio = mysql_query($sql_resumo_simposio, $conexao) or die(mysql_error());
	$existe_resumo_simposio = mysql_num_rows($resumo_simposio);
	...
	// VERIFICAR SE É AUTOR DE TRABALHO EM SESSÃO DE COMUNICAÇÃO COORDENADA
	// ??? - Confirmar com a ALAB qual o tipo de carta que será gerada caso seja aceito...

	// COMUNICAÇÃO INDIVIDUAL
	$sql_individual = "SELECT r.id
								  FROM ev_resumo r, ev_tipo_trabalho tt
								  WHERE  r.id_tipo_trabalho = tt.id AND
								  (r.autor='".$id_participante."' OR r.co_autor='".$id_participante."') AND 
								  r.id_evento='".$id_evento."' AND
	...
	// PôSTER
	$sql_poster = "SELECT r.id, r.titulo
				  FROM ev_resumo r, ev_tipo_trabalho tt
				  WHERE  r.id_tipo_trabalho = tt.id AND
				  (r.autor='".$id_participante."' OR r.co_autor='".$id_participante."') AND 
				  r.id_evento='".$id_evento."' AND
      ...
      <div id="artigo">
         <div id="box_trabalhos"><span class="txt_titulo_destaque">Carta de Aceite</span>
           ...
           <?php 
			...
			// [?] AUTOR DE TRABALHO EM SESSÃO DE COMUNICAÇÃO COORDENADA 
			// COMUNICAÇÃO INDIVIDUAL
			// PôSTER
		
		   $categorias_carta = array(
		   		array("categoria_carta" => "coordenador_simposio", "existe" => $existe_simposio,           "registros_bd" => $simposio,           "label" => "Coordenador de Simp&oacute;sio"),
		   		array("categoria_carta" => "resumo_simposio", "existe" => $existe_resumo_simposio,    "registros_bd" => $resumo_simposio,    "label" => "Resumo em Simp&oacute;sio"),
		   		array("categoria_carta" => "debatedor", "existe" => $existe_debatedor_simposio, "registros_bd" => $debatedor_simposio, "label" => "Debatedor de Simp&oacute;sio"),
		   		array("categoria_carta" => "coordenador_coordenada", "existe" => $existe_coordenada, "registros_bd" => $coordenada, "label" => "Comunica&ccedil;&atilde;o Coordenada"),
		   		array("categoria_carta" => "comunicacao_individual", "existe" => $existe_individual, "registros_bd" => $individual, "label" => "Comunica&ccedil;&atilde;o Individual"),
		   		array("categoria_carta" => "poster", "existe" => $existe_poster,     "registros_bd" => $poster,     "label" => "P&ocirc;ster")
		   );
		   foreach($categorias_carta as $categ_carta){
			   if ($categ_carta['existe'] > 0){
						while($mostrar = mysql_fetch_array($categ_carta['registros_bd'])){ ?>
