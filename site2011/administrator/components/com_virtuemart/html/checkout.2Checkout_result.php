...
*
* @version $Id: checkout.2Checkout_result.php 3469 2011-06-09 08:22:12Z zanardi $
* @package VirtueMart
* @subpackage html
...
if( !isset( $_REQUEST["x_invoice_num"] ) || empty( $_REQUEST["x_invoice_num"] )) {
	...
	/* x_invoic_num is the name of the variable that holds OUR order_number */
	$order_number = $_REQUEST['x_invoice_num'];
	...
	if ($compare_hash1 != $compare_hash2) { 
        ...
        $qv .= "WHERE order_number='".$dbbt->getEscaped($order_number)."'";
        $dbbt->query($qv);
        $dbbt->next_record();
		...
		if ($_REQUEST['x_2checked'] == 'Y') {
            ...
            <h2><?php echo $VM_LANG->_('PHPSHOP_PAYMENT_TRANSACTION_SUCCESS') ?></h2><?php
        } else {
