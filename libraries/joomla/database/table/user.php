...
// Somente para o administrador saber qual estado e cidade colocar
	var $estado_res	= null;
...
/*		if (trim( $this->id_estado_res ) == '') {

			$this->setError( JText::_( 'Selecione um estado.') );

			return false;

		}
		
		if (trim( $this->id_cidade_res ) == '') {

			$this->setError( JText::_( 'Selecione uma cidade.') );
...
/*		if (trim( $this->cep_res ) == '') {
...

	{
		...
		if($this->get("cpf") != ''){
			$this->cpf = ereg_replace("[^0-9]", "", $this->cpf);
		}
		
		$anuidade1 = $this->Anuidade1; // ano atual
		$anuidade2 = $this->Anuidade2;
		$anuidade3 = $this->Anuidade3;
		
		unset($this->Anuidade1);
		unset($this->Anuidade2);
		unset($this->Anuidade3);
...

		{
			...
			############### Anuidade Ano Atual ##################################
			if(strlen($anuidade1)<6){
				$select_anuidade1 = "SELECT id_ano FROM #__anos WHERE nome = '".$anuidade1."'";
				$db->setQuery($select_anuidade1);
				$res1 = $db->loadResult ();
				$select_exist1 = "SELECT * FROM #__anos_user WHERE id_ano = '".$res1['id_ano']."' AND id_user = '".(int) $this->$k."'";
				$db->setQuery($select_exist1);
				$db->query();
				$rows1 = $db->getNumRows();
				if($rows1 == 0){
					$select_anuidade1 = "INSERT INTO #__anos_user (id_ano, id_user) VALUES ('".$res1['id_ano']."', '".(int) $this->$k."')";
					$db->setQuery($select_anuidade1);
					$db->query();
				}
			}
				...
				$anuidade1 = str_replace("NO","",$anuidade1);
				$select_anuidade1 = "SELECT id_ano FROM #__anos WHERE nome = '".$anuidade1."'";
				$db->setQuery($select_anuidade1);
				$res1 = $db->loadResult ();
				$select_anuidade1 = "DELETE FROM #__anos_user WHERE id_user = '".(int) $this->$k."' AND id_ano = '".$res1['id_ano']."'";
				$db->setQuery($select_anuidade1);
				$db->query();
			}
			#################################################################
			############## Anuidade (Ano Atual - 1) #########################
			if(strlen($anuidade2)<6){
				$select_anuidade2 = "SELECT id_ano FROM #__anos WHERE nome = '".$anuidade2."'";
				$db->setQuery($select_anuidade2);
				$res2 = $db->loadResult ();
				$select_exist2 = "SELECT * FROM #__anos_user WHERE id_ano = '".$res2['id_ano']."' AND id_user = '".(int) $this->$k."'";
				$db->setQuery($select_exist2);
				$db->query();
				$rows2 = $db->getNumRows();
				if($rows2 == 0){
					$select_anuidade2 = "INSERT INTO #__anos_user (id_ano, id_user) VALUES ('".$res2['id_ano']."', '".(int) $this->$k."')";
					$db->setQuery($select_anuidade2);
					$db->query();
				}
			}
				...
				$anuidade2 = str_replace("NO","",$anuidade2);
				$select_anuidade2 = "SELECT id_ano FROM #__anos WHERE nome = '".$anuidade2."'";
				$db->setQuery($select_anuidade2);
				$res2 = $db->loadResult ();
				$select_anuidade2 = "DELETE FROM #__anos_user WHERE id_user = '".(int) $this->$k."' AND id_ano = '".$res2['id_ano']."'";
				$db->setQuery($select_anuidade2);
				$db->query();
			}
			#################################################################
			############## Anuidade (Ano Atual - 2) #########################
			if(strlen($anuidade3)<6){
				$select_anuidade3 = "SELECT id_ano FROM #__anos WHERE nome = '".$anuidade3."'";
				$db->setQuery($select_anuidade3);
				$res3 = $db->loadResult ();
				$select_exist3 = "SELECT * FROM #__anos_user WHERE id_ano = '".$res3['id_ano']."' AND id_user = '".(int) $this->$k."'";
				$db->setQuery($select_exist3);
				$db->query();
				$rows3 = $db->getNumRows();
				if($rows3 == 0){
					$select_anuidade3 = "INSERT INTO #__anos_user (id_ano, id_user) VALUES ('".$res3['id_ano']."', '".(int) $this->$k."')";
					$db->setQuery($select_anuidade3);
					$db->query();
				}
			}
				...
				$anuidade3 = str_replace("NO","",$anuidade3);
				$select_anuidade3 = "SELECT id_ano FROM #__anos WHERE nome = '".$anuidade3."'";
				$db->setQuery($select_anuidade3);
				$res3 = $db->loadResult ();
				$select_anuidade3 = "DELETE FROM #__anos_user WHERE id_user = '".(int) $this->$k."' AND id_ano = '".$res3['id_ano']."'";
				$db->setQuery($select_anuidade3);
				$db->query();
			}
...

		{
			...
			$select_anuidade1 = "DELETE FROM #__anos_user WHERE id_user = '".(int) $this->$k."'";
			$db->setQuery($select_anuidade1);
			$db->query();
...

	{
		...
		// Excluidno dados referente aou usuario excluido da tabela de anuidades.
		$exclui_anuidade = "DELETE FROM #__anos_user WHERE id_user = '".(int) $this->$k."'";
		$this->_db->setQuery($exclui_anuidade);
		$this->_db->query();
