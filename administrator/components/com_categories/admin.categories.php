...
function editCategory($edit )
{
	...
	// aqui faço o select para listar os tipos de evento
	$query = 'SELECT id AS value, nome AS text FROM ev_tipo_evento';
	$db->setQuery($query);
	$tipos_evento = $db->loadObjectList();
	$lists['tipo_evento'] = JHTML::_('select.genericlist', $tipos_evento, 'tipo_evento', 'class="inputbox" size="1"', 'value', 'text', $row->tipo_evento);
...
function saveCategory()
{
	...
	// Testo se tá rolando um INSERT na tabela #__categories para então poder criar um evento para o sistema de eventos
	if ($row->id) {
		$db->setQuery('INSERT INTO ev_evento (id_categ_joomla, titulo) VALUES ('.$row->id.', \''.$row->title.'\')');
		$db->query();
	}
	
	// Atualizar o evento nas tabelas do sistema de eventos
	$db->setQuery('UPDATE ev_evento SET titulo = \''.$row->title.'\' WHERE id_categ_joomla = '.$row->id);
	$db->query();
