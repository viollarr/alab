...
if(!defined('_RSFORM_REVISION'))
	...
	function RSgetComponentProperties($componentId)
	{
		...
		$rez = mysql_query("SELECT PropertyName, PropertyValue FROM `$RSadapter->tbl_rsform_properties` WHERE ComponentId=$componentId");
		$data=array();
		while($r=mysql_fetch_assoc($rez))
			$data[$r['PropertyName']]=$r['PropertyValue'];
	...
	function RSprocessForm($formId)
	{
		...
		if(empty($invalid))
		{
			...
			if(isset($_FILES['form']['tmp_name']) && is_array($_FILES['form']['tmp_name']))
			{
				...
				for($i=0;$i<count($dest);$i++)
					if(isset($tmp_name[$i]))
					{
						...
						mysql_query("INSERT INTO `{$RSadapter->tbl_rsform_submission_values}` (`SubmissionId`, `FieldName`, `FieldValue`) VALUES ('{$SubmissionId}','$fieldName[$i]','$db')");
					}
			}
			...
			foreach ($_POST['form'] as $key=>$val)
			{
				...
				mysql_query("INSERT INTO `{$RSadapter->tbl_rsform_submission_values}` (`SubmissionId`, `FieldName`, `FieldValue`) VALUES ('{$SubmissionId}','".$key."','".$val."')");
			}
	...
	function RSprocessField($result,$submissionId)
	{
		...
		//get form id
		$Submission = mysql_fetch_assoc(mysql_query("SELECT * FROM $RSadapter->tbl_rsform_submissions WHERE SubmissionId = '$submissionId'"));
		...
		while ($Component = mysql_fetch_assoc($query))
		{
			...
			$SubmissionValue = @mysql_result(mysql_query("SELECT FieldValue FROM $RSadapter->tbl_rsform_submission_values WHERE FieldName = '{$properties['NAME']}' AND SubmissionId = '$submissionId'"),0);
			if ($SubmissionValue === false) $SubmissionValue = '';
			if ($SubmissionValue !== false && RSgetComponentTypeId($Component['ComponentId'])==9) $SubmissionValue = basename($SubmissionValue);
...
<tasks></tasks>
    ...
    */
    //LOAD FORMS
    $query_forms = mysql_query("SELECT * FROM `{$RSadapter->tbl_rsform_forms}` WHERE FormId IN ('".implode("','",$formIds)."') ORDER BY FormId");
    while($form_row = mysql_fetch_array($query_forms,MYSQL_ASSOC))
    {
         ...
         //LOAD COMPONENTS
	    $query_components = mysql_query("SELECT * FROM `$RSadapter->tbl_rsform_components` WHERE FormId = '".$form_row['FormId']."'");
	    while($component_row = mysql_fetch_array($query_components,MYSQL_ASSOC))
	    {
	         ...
	         $tasks[] = '<task type="eval" source="">$GLOBALS[\'q_ComponentId\'] = mysql_insert_id();</task>';
	             ...
	             //LOAD PROPERTIES
			    $query_properties = mysql_query("SELECT * FROM `{$RSadapter->tbl_rsform_properties}` WHERE ComponentId = '".$component_row['ComponentId']."'");
			    while($property_row = mysql_fetch_array($query_properties,MYSQL_ASSOC))
			    {
		...
		{
		    //LOAD SUBMISSIONS
		    $query_submissions = mysql_query("SELECT * FROM `{$RSadapter->tbl_rsform_submissions}` WHERE FormId = '".$form_row['FormId']."'");
		    while($submission_row = mysql_fetch_array($query_submissions,MYSQL_ASSOC))
		    {
				...
				$query_submission_value = mysql_query("SELECT * FROM `{$RSadapter->tbl_rsform_submission_values}` WHERE SubmissionId = '".$submission_row['SubmissionId']."'");
				while($submission_value_row = mysql_fetch_array($query_submission_value,MYSQL_ASSOC))
			    {
