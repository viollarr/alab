...
class ctrl_presenca extends ctrl_generico {
    ...
    function presencas_qp4_participante() {
        ...
        //////////////////
        //$participante = array();
        $sql = "SELECT part.id, part.nome, pre.presente, 
            CASE part.id_tipo_participante 
                WHEN 13 THEN 'sim'
                ELSE 'nao'
            END AS ouvinte
            FROM ev_participante AS part
        ...
        $sql = "SELECT c.id, c.titulo_sessao AS titulo, p.presente 
                FROM ev_comunicacao_coordenada AS c
                LEFT JOIN ev_presenca p ON c.id = p.id_trabalho 
                    AND p.id_evento = '{$this->id_evento}' 
                    AND p.tipo = 'coordenacao_sessao' 
                    AND p.id_participante = '{$id_participante}'
                WHERE
                        c.id_coordenador = " . $id_participante . "
                        AND c.id_evento = " . $this->id_evento;
        $result = mysql_query($sql);
        ...
        $sql = "SELECT resumo.id, resumo.titulo, p.presente 
                ...
                LEFT JOIN ev_presenca p ON participante_resumo.id_resumo = p.id_trabalho 
                    AND p.id_evento = '{$this->id_evento}' 
                    AND p.tipo = 'comunicacao_coordenada' 
                    AND p.id_participante = '{$id_participante}'
                WHERE participante_resumo.id_participante = $id_participante
                        AND participante_resumo.id_resumo = resumo.id
                        AND resumo.id_tipo_trabalho = 2
    ...
    function salvar_presenca_qp4() {
        ...
        $certificados_possiveis = array('comunicacao_individual', 'coordenacao_sessao', 'comunicacao_coordenada', 'ouvinte');
