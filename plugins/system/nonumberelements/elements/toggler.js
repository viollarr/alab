...
if ( typeof( nn_toggler_version ) == 'undefined' || nn_toggler_version < 1002008 ) {
	...
	var nnToggler = new Class({
		...
		initTogglers: function( id )
		{
			...
			this.autoHeightDivs();
			( function() { self.autoHeightDivs(); } ).delay( 500 );
		},
		...
		setElementAction : function( element_name, toggler )
		{
			...
			this.form_elements.each( function( el ) {
				if (
					...
					||	el.name == 'advancedparams['+element_name+']' || el.name == 'advancedparams['+element_name+'][]'
