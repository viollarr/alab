...
*/
/**
* @version $Id: mod_latestnews.php 5071 2006-09-15 16:16:55Z friesengeist $
* @package Joomla
...
switch ( $type ) {
		...
		if (count($temp)) {
			foreach ($temp as $row ) {
				if(isset($my->jaclplus)){
					if (($row->cat_state == 1 || $row->cat_state == '') &&  ($row->sec_state == 1 || $row->sec_state == '') &&  (in_array( $row->cat_access, explode( ",", $my->jaclplus ) ) || $row->cat_access == '' || !$access) &&  (in_array( $row->sec_access, explode( ",", $my->jaclplus ) ) || $row->sec_access == '' || !$access)) {
						$rows[] = $row;
					}
				} else {
	...
	//Content Items only
		...
		. ( $access ? "\n AND " . ( isset($my->jaclplus) ? "a.access IN ( $my->jaclplus ) AND cc.access IN ( $my->jaclplus ) AND s.access IN ( $my->jaclplus )" : "a.access <= " . (int) $my->gid . " AND cc.access <= " . (int) $my->gid . " AND s.access <= " . (int) $my->gid ) : '' )
		. $whereCatid
		. $whereSecid
		. ( $show_front == '0' ? "\n AND f.content_id IS NULL" : '' )
		. "\n AND s.published = 1"
