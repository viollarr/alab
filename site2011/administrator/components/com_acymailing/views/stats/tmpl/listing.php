		...
		<tbody>
			...
			?>
				<tr class="<?php echo "row$k"; ?>">
					...
					<td align="center">
					<?php if(acymailing::level(2)){ ?><a class="modal" href="<?php echo acymailing::completeLink('diagram&task=mailing&mailid='.$row->mailid,true)?>" rel="{handler: 'iframe', size: {x: 800, y: 580}}"><?php } ?>
					<?php echo $row->subject; ?>
					<?php if(acymailing::level(2)) echo '</a>'; ?>
					</td>
					<td align="center">
						<?php
						if(!empty($row->senthtml)){
							...
							echo acymailing::tooltip( $text, $title, '', $pourcent,acymailing::completeLink('stats&task=detaillisting&filter_status=open&filter_mail='.$row->mailid));
						}
						?>
					</td>
					<?php if(acymailing::level(1)){?>
					<td align="center">
						...
						if(!empty($row->senthtml)){
							...
							echo acymailing::tooltip( $text, $title, '', $pourcent,acymailing::completeLink('statsurl&filter_mail='.$row->mailid));
						}
						?>
					</td>
					<?php }
					if(acymailing::level(2)){?>
						<td align="center">
							...
							echo acymailing::tooltip( $row->unsub, $pourcent, '', $pourcent);?>
						</td>
						<td align="center">
							<?php echo $row->forward; ?>
						</td>
					<?php } ?>
					<td align="center">
					...
					<td align="center">
						...
						echo acymailing::tooltip( $row->bounceunique, $pourcent, '', $pourcent,acymailing::completeLink('stats&task=detaillisting&filter_status=bounce&filter_mail='.$row->mailid));?>
					</td>
					<?php } ?>
					<td align="center">
						<a href="<?php echo acymailing::completeLink('stats&task=detaillisting&filter_status=failed&filter_mail='.$row->mailid); ?>">
						<?php echo $row->fail; ?>
						</a>
					</td>
					<td align="center">
