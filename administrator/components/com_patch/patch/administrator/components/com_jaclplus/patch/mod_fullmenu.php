	...
	*/
	...
	<?php
			...
			$topLevelCount = 0;
				$com_string = "";
			foreach ($comps as $row) {
					if ($editAllComponents | $acl->acl_check( 'administration', 'edit', 'users', $usertype, 'components', $row->option ) | $acl->acl_check( 'administration', 'manage', 'users', $usertype, 'components', $row->option )) {
					if ($row->parent == 0 && (trim( $row->admin_menu_link ) || array_key_exists( $row->id, $subs ))) {
						...
						$link = $row->admin_menu_link ? "'index2.php?$row->admin_menu_link'" : "null";
							$com_string .= "\t\t\t\t['<img src=\"../includes/$row->admin_menu_img\" />','$name',$link,null,'$alt'";
						if (array_key_exists( $row->id, $subs )) {
							foreach ($subs[$row->id] as $sub) {
								...
								$link = $sub->admin_menu_link ? "'index2.php?$sub->admin_menu_link'" : "null";
									$com_string .= "\t\t\t\t\t['<img src=\"../includes/$sub->admin_menu_img\" />','$name',$link,null,'$alt']";
							}
						}
							$com_string .= "\n\t\t\t\t],\n";
					}
			...
			if ($topLevelLimit < $topLevelCount) {
					$com_string .= "\t\t\t\t['<img src=\"../includes/js/ThemeOffice/sections.png\" />','More Components...','index2.php?option=com_admin&task=listcomponents',null,'More Components'],\n";
			}
	?>
<?php
	// Components Sub-Menu
...
?>	
			_cmSplit,
			[null,'Components',null,null,'Menu inactive for this Page'
			],
	<?php
	...
	<?php
		// Installer Sub-Menu
	if ($manageTemplates || $manageLanguages || $installComponents || $installModules || $installMambots) {
	?>			_cmSplit,
		...
		*/
		...
		/* Mambots Sub-Menu */
			if ( $installMambots || $editAllMambots) {
				...
				<?php
		/* Installer Sub-Menu */
			if ($installComponents || $installModules || $installMambots) {
					?>
					_cmSplit,
