...
class plgAcymailingTagcbuser extends JPlugin
{
	...
	function onAcyProcessFilter_cbfield(&$query,$filter,$num){
	 ...
	 function acymailingtagcb_show(){
		...
		$text .= '<tr style="cursor:pointer" class="row1" onclick="setTag(\'{cbtag:thumb}\');insertTag();" ><td class="acytdcheckbox"></td><td>Thumb Avatar</td></tr>';
		foreach($fields as $fieldname => $oneField){
			...
			$text .= '<tr style="cursor:pointer" class="row'.$k.'" onclick="setTag(\'{cbtag:'.$fieldname.$type.'}\');insertTag();" ><td class="acytdcheckbox"></td><td>'.$fieldname.'</td></tr>';
			$k = 1-$k;
		}
		...
		foreach($otherFields as $oneField){
			$text .= '<tr style="cursor:pointer" class="row'.$k.'" onclick="setTag(\'{cbtag:cbapi_'.$oneField->name.'}\');insertTag();" ><td class="acytdcheckbox"></td><td>'.$oneField->name.'</td></tr>';
			$k = 1-$k;
		}
		$text .= '</table>';
		echo $text;
