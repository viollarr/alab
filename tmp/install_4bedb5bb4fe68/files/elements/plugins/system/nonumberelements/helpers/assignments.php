...
class NoNumberElementsAssignmentsHelper
{
	function NoNumberElementsAssignmentsHelper()
	{
		...
		$this->_params->assignment_types = array(
			'MenuItem',
			'SecsCats',
			'Categories_K2',
			'Articles',
		...
		$this->_params->db =& JFactory::getDBO();
	...
	function initParams( &$params, $type = '' )
	{
		...
		if ( !isset( $params->selection ) || ( !is_array( $params->selection ) && trim( $params->selection ) == '' ) ) {
	...
	function passAll( &$params, $match_method = 'and', $article = 0 )
	{
		...
		foreach ( $this->_params->assignment_types as $type ) {
			...
			foreach ( $selection as $seccat ) {
				...
				if ( count( $seccat ) > 1 ) {
					...
					if ( $params->inc_children ) {
						...
						$this->_params->db->setQuery( $query );
						$categories = $this->_params->db->loadResultArray();
			...
			switch ( $this->_params->view ) {
				...
				default:
					$query = 'SELECT catid'
						...
						.' LIMIT 1';
					$this->_params->db->setQuery( $query );
					$cats = $this->_params->db->loadResult();
					break;
			}
		}
	...
	function getMenuItemParentIds( $menu_id = 0 )
	{
		...
		while ( $menu_id ) {
			$query = 'SELECT parent'
				...
				.' LIMIT 1';
			$this->_params->db->setQuery( $query );
			$menu_id = $this->_params->db->loadResult();
			if ( $menu_id ) {
				$parent_ids[] = $menu_id;
			}
		}
	...
	function getK2CatParentIds( $cat_id = 0 )
	{
		...
		while ( $cat_id ) {
			$query = 'SELECT parent'
				...
				.' LIMIT 1';
			$this->_params->db->setQuery( $query );
			$cat_id = $this->_params->db->loadResult();
			if ( $cat_id ) {
				$parent_ids[] = $cat_id;
			}
		}
