     ...
     <?php if ($modalidade_participacao==1) {//1 -> com trabalho //1 -> é associado alab?>
       ...
       <br />
		...
		$sql = "SELECT id, id_tipo_trabalho, titulo FROM ev_resumo 
			WHERE autor='".$_SESSION['id_participante']."'";
		...
		$sql = "SELECT id, id_tipo_trabalho, titulo FROM ev_resumo 
			WHERE co_autor='".$_SESSION['id_participante']."'";
		$result = mysql_query($sql, $conexao);
		while($resumo = mysql_fetch_array($result)){
			$trabalhos_cadastrados[$j]["id"]               = $resumo["id"];
			$trabalhos_cadastrados[$j]["id_tipo_trabalho"] = $resumo["id_tipo_trabalho"];
			...
			foreach($trabalhos_cadastrados as $trabalho_cadastrado){
				...
				*/
				...
				switch($trabalho_cadastrado["id_tipo_trabalho"]){
					case 1:
						if($trabalho_cadastrado["eh_coordenador"]) echo "Coordenador do Simpósio ".$trabalho_cadastrado["titulo"];
						elseif($trabalho_cadastrado["eh_autor"]) echo "Autor do trabalho ".$trabalho_cadastrado["titulo"]." em um Simpósio.";
						elseif($trabalho_cadastrado["eh_co_autor"]) echo "Co-autor do trabalho ".$trabalho_cadastrado["titulo"]." em um Simpósio.";
						break;
					
					case 2:
						if($trabalho_cadastrado["eh_coordenador"]) echo "Coordenador da Comunicação Coordenada ".$trabalho_cadastrado["titulo"];
						elseif($trabalho_cadastrado["eh_autor"]) echo "Autor do trabalho ".$trabalho_cadastrado["titulo"]." em uma Comunicação Coordenada.";
						elseif($trabalho_cadastrado["eh_co_autor"]) echo "Co-autor do trabalho ".$trabalho_cadastrado["titulo"]." em uma Comunicação Coordenada.";
						break;
					
					case 3:
						if($trabalho_cadastrado["eh_autor"]) echo "Autor da Comunicação Individual ".$trabalho_cadastrado["titulo"].".";
						elseif($trabalho_cadastrado["eh_co_autor"]) echo "Co-autor da Comunicação Individual ".$trabalho_cadastrado["titulo"].".";
						break;
					
					case 4:
						if($trabalho_cadastrado["eh_autor"]) echo "Autor do Pôster ".$trabalho_cadastrado["titulo"].".";
						elseif($trabalho_cadastrado["eh_co_autor"]) echo "Co-autor do Pôster ".$trabalho_cadastrado["titulo"].".";
						break;
				}
				echo "</li>";
			}//foreach
