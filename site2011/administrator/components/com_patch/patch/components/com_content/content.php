...
if(class_exists('JACLPlus')) {
	require_once( $mosConfig_absolute_path . '/administrator/components/com_jaclplus/jaclplus.config.php' );
	$_JACL = new jaclConfig();
} else {
	$_JACL = new stdClass();
	$_JACL->enable_jaclplus = false;
	$_JACL->auto_disablecache = false;
	$_JACL->limit_edit = false;
	$_JACL->limit_editown = false;
	$_JACL->edit_accessonly = false;
	$_JACL->publish_frontpage = true;
	$_JACL->require_publish = false;
}
...
if ( $option == 'com_frontpage' ) {
	if( $my->id && $_JACL->enable_jaclplus && $_JACL->auto_disablecache ) {
...
switch ( $task ) {
	...
	case 'view':
		if ($mosConfig_enable_stats || ( $my->id && $_JACL->enable_jaclplus && $_JACL->auto_disablecache ) ) {
	...
	case 'category':
		...
		if( $my->id && $_JACL->enable_jaclplus && $_JACL->auto_disablecache ) {
			showCategory( $id, $gid, $access, $sectionid, $limit, NULL, $limitstart, 0, $selected, $filter );
		} else {
	...
	case 'blogsection':
		...
		if( $my->id && $_JACL->enable_jaclplus && $_JACL->auto_disablecache ) {
			showBlogSection( $id, $gid, $access, $pop, 0, $limit, $limitstart );
		} else {
	...
	case 'blogcategory':
		// Itemid is a dummy value to cater for caching
		if( $my->id && $_JACL->enable_jaclplus && $_JACL->auto_disablecache ) {
			showBlogCategory( $id, $gid, $access, $pop, 0, $limit, $limitstart );
		} else {
	...
	case 'archivesection':
		// Itemid is a dummy value to cater for caching
		if( $my->id && $_JACL->enable_jaclplus && $_JACL->auto_disablecache ) {
			showArchiveSection( $id, $gid, $access, $pop, $option, $year, $month, $limit, $limitstart );
		} else {
	...
	case 'archivecategory':
		// Itemid is a dummy value to cater for caching
		if( $my->id && $_JACL->enable_jaclplus && $_JACL->auto_disablecache ) {
			showArchiveCategory( $id, $gid, $access, $pop, $option, $year, $month, $module, $limit, $limitstart );
		} else {
...
function showSection( $id, $gid, &$access, $now ) {
	global $database, $mainframe, $Itemid, $my, $acl, $_JACL;
	...
	*/
	if(class_exists('JACLPlus')) $deny = !JACLPlus::InList($section->access, $my->jaclplus);
	else $deny = $section->access > $gid;
	...
	if(!$access->canEdit && $my->id && $_JACL->enable_jaclplus) $access->canEdit = $acl->acl_check( 'action', 'edit', 'users', $my->usertype, 'section', $id );
	...
	if ( $access->canEdit ) {
		...
		$extra = extraCondition('1', 'b');
		$xwhere = "\n AND (a.published = '1' ".$extra[1]." )";
		$xwhere2 = "\n AND ((b.state = 1"
		. "\n AND ( b.publish_up = " . $database->Quote( $nullDate ) . " OR b.publish_up <= " . $database->Quote( $now ) . " )"
		. "\n AND ( b.publish_down = " . $database->Quote( $nullDate ) . " OR b.publish_down >= " . $database->Quote( $now ) . " )) ".$extra[0].")"
		;
	}
...
function showCategory( $id, $gid, &$access, $sectionid, $limit, $selected, $limitstart, $now, $selected, $filter ) {
	global $database, $mainframe, $Itemid, $mosConfig_list_limit, $my, $acl, $_JACL;
	...
	*/
	if(class_exists('JACLPlus')) $deny = !JACLPlus::InList($category->access, $my->jaclplus);
	else $deny = $category->access > $gid;
	...
	*/
	...
	if(!$access->canEdit && $my->id && $_JACL->enable_jaclplus) $access->canEdit = $acl->acl_check( 'action', 'edit', 'users', $my->usertype, 'section', $sectionid );
	...
	. $xwhere2
	//. ( $noauth ? "\n AND b.access <= " . (int) $gid : '' )
	. ( $noauth ? "\n AND b.access ".(isset( $my->jaclplus ) ? "IN ( $my->jaclplus )" : "<= " . (int) $gid) : '' )
	. "\n WHERE c.section = '" . (int) $category->section . "'"
	. $xwhere
	//. ( $noauth ? "\n AND c.access <= " . (int) $gid : '' )
	. ( $noauth ? "\n AND c.access ".(isset( $my->jaclplus ) ? "IN ( $my->jaclplus )" : "<= " . (int) $gid) : '' )
	. "\n GROUP BY c.id"
	. $empty
	. "\n ORDER BY c.ordering"
	;
	...
	if(!$access->canEdit && $my->id && $_JACL->enable_jaclplus) $access->canEdit 	= $acl->acl_check( 'action', 'edit', 'users', $my->usertype, 'category', $id );
	...
	if ( $access->canEdit ) {
		...
		$xwhere = "\n AND ( ( a.state = 1"
		. "\n AND ( publish_up = " . $database->Quote( $nullDate ) . " OR publish_up <= " . $database->Quote( $now ) . " )"
		. "\n AND ( publish_down = " . $database->Quote( $nullDate ) . " OR publish_down >= " . $database->Quote( $now ) . " ) ) ".$extra[0]." )"
		;
	}
	...
	. ( $noauth ? "\n AND a.access ".(isset( $my->jaclplus ) ? "IN ( $my->jaclplus )" : "<= " . (int) $gid) : '' )
	. "\n AND " . (int) $category->access . (isset( $my->jaclplus ) ? " IN ( $my->jaclplus )" : " <= " . (int) $gid)
	. $and
	. "\n ORDER BY $orderby"
	;
	$database->setQuery( $query );
	...
	if(!$access->canAdd && $my->id && $_JACL->enable_jaclplus) {
		$access->canAdd = $acl->acl_check( 'action', 'add', 'users', $my->usertype, 'content', 'all' );
		if(!$access->canAdd) {
			$access->canAdd = $acl->acl_check( 'action', 'add', 'users', $my->usertype, 'section', $sectionid );
			if(!$access->canAdd) $access->canAdd = $acl->acl_check( 'action', 'add', 'users', $my->usertype, 'category', $id );
		}
	}
...
function showBlogSection( $id=0, $gid, &$access, $pop, $now=NULL, $limit, $limitstart ) {
	...
	if (!count($rows) && $check) {
		...
		* check whether section access level allows access
		*/
		if(class_exists('JACLPlus')) $deny = !JACLPlus::InList($secCheck->access, $my->jaclplus);
		else $deny = $secCheck->access > $gid;
...
function showBlogCategory( $id=0, $gid, &$access, $pop, $now, $limit, $limitstart ) {
	...
	if (!count($rows) && $check) {
		...
		* check whether category access level allows access
		*/
		if(class_exists('JACLPlus')) $deny = !JACLPlus::InList($catCheck->access, $my->jaclplus);
		else $deny = $catCheck->access > $gid;
...
function showItem( $uid, $gid, &$access, $pop, $option='com_content', $now ) {
	...
	$now 		= _CURRENT_SERVER_TIME;
	if($my->id && $_JACL->enable_jaclplus) { // login user only
		$query = "SELECT id, catid, sectionid FROM #__content WHERE id = " . (int) $uid;
		$database->setQuery( $query );
		$row = NULL;
		if ( $database->loadObject( $row ) ) {
			if(!$access->canEdit) {
				$access->canEdit 	= $acl->acl_check( 'action', 'edit', 'users', $my->usertype, 'section', $row->sectionid );
				if(!$access->canEdit) {
					$access->canEdit 	= $acl->acl_check( 'action', 'edit', 'users', $my->usertype, 'category', $row->catid );
					if(!$access->canEdit) $access->canEdit 	= $acl->acl_check( 'action', 'edit', 'users', $my->usertype, 'content', $row->id );
				}
			}
		}
	}
	...
	if ( $access->canEdit ) {
		$xwhere = '';
	} else if ( $my->id && $_JACL->enable_jaclplus && $access->canEditOwn ) {
		$xwhere = '';
	} else {
	...
	if ( $database->loadObject( $row ) ) {
		...
		* check whether category access level allows access
		*/
		if(class_exists('JACLPlus')) $deny = !JACLPlus::InList($row->cat_access, $my->jaclplus);
		else $deny = $row->cat_access > $gid;
		...
		* check whether section access level allows access
		*/
		if(class_exists('JACLPlus')) $deny = !JACLPlus::InList($row->sec_access, $my->jaclplus);
		else $deny = $row->sec_access > $gid;
		//if ( ($row->sec_access > $gid) && $row->sectionid ) {
		if ( $deny && $row->sectionid ) {
			mosNotAuth();
			return;
		}
...
function show( $row, $params, $gid, &$access, $pop, $option='com_content', $ItemidCount=NULL ) {
	...
	$noauth = !$mainframe->getCfg( 'shownoauth' );
	$default_canEdit = $access->canEdit;
	$default_canEditOwn = $access->canEditOwn;
	if($_JACL->enable_jaclplus && !$access->canEdit && $my->id) {
		$access->canEdit = $acl->acl_check( 'action', 'edit', 'users', $my->usertype, 'section', $row->sectionid );
		if(!$access->canEdit) {
			$access->canEdit = $acl->acl_check( 'action', 'edit', 'users', $my->usertype, 'category', $row->catid );
			if(!$access->canEdit) $access->canEdit 	= $acl->acl_check( 'action', 'edit', 'users', $my->usertype, 'content', $row->id );
	...
	if ( $access->canEdit ) {
		if(class_exists('JACLPlus')) $deny = !JACLPlus::InList( $row->access, $my->jaclplus );
		else $deny = $row->access > $gid;
		//if ( $row->id === null || $row->access > $gid ) {
		if ( $row->id === null || $deny ) {
			//mosNotAuth();
			//return;
			if ( $_JACL->enable_jaclplus && $_JACL->edit_accessonly ) $access->canEdit = false;
			if ( $noauth ) {
			mosNotAuth();
			return;
			} else {
	...
	} else if ( $my->id && $_JACL->enable_jaclplus && $access->canEditOwn && $row->created_by == $my->id ) {
		if ( $row->id === null || !JACLPlus::InList( $row->access, $my->jaclplus ) ) {
			//mosNotAuth();
			//return;
			if ( $_JACL->edit_accessonly ) $access->canEditOwn = false;
			if ( $noauth ) {
				mosNotAuth();
				return;
			} else {
	...
	$access->canEdit = $default_canEdit;
	$access->canEditOwn = $default_canEditOwn;
}


function editItem( $uid, $gid, &$access, $sectionid=0, $task, $Itemid ){
	global $database, $my, $mainframe, $acl, $_JACL;
	...
	$cid = 0;
	if ( $uid ) {
		if ( $_JACL->enable_jaclplus && $_JACL->edit_accessonly ) {
			if ( !JACLPlus::InList( $row->access, $my->jaclplus ) ) {
				mosNotAuth();
				return;
			}
		}
		if(!$access->canEdit && $_JACL->enable_jaclplus) {
			$access->canEdit = $acl->acl_check( 'action', 'edit', 'users', $my->usertype, 'section', $row->sectionid );
			if(!$access->canEdit) {
				$access->canEdit = $acl->acl_check( 'action', 'edit', 'users', $my->usertype, 'category', $row->catid );
				if(!$access->canEdit) $access->canEdit = $acl->acl_check( 'action', 'edit', 'users', $my->usertype, 'content', $row->id );
				$cid = intval( $row->catid );
				if($access->canEdit || $access->canEditOwn) $from_cat = true;
			}
		}
		// existing record
		...
		if(!$access->canPublish && $_JACL->enable_jaclplus) {
			$access->canPublish = $acl->acl_check( 'action', 'publish', 'users', $my->usertype, 'section', $row->sectionid );
			if(!$access->canPublish) {
				$access->canPublish = $acl->acl_check( 'action', 'publish', 'users', $my->usertype, 'category', $row->catid );
				if(!$access->canPublish) {
					$access->canPublish = $acl->acl_check( 'action', 'publish', 'users', $my->usertype, 'content', $row->id );
					if(!$access->canPublish && $row->created_by == $my->id) $access->canPublish = $acl->acl_check( 'action', 'publish', 'users', $my->usertype, 'content', 'own' );
				}
			}
		}
	} else {
		if(!$access->canEdit && $_JACL->enable_jaclplus) {
			$access->canEdit 	= $acl->acl_check( 'action', 'add', 'users', $my->usertype, 'content', 'all' );
			if(!$access->canEdit) {
				$access->canEdit 	= $acl->acl_check( 'action', 'add', 'users', $my->usertype, 'section', $sectionid );
				if(!$access->canEdit && !$_JACL->limit_edit) $access->canEdit 	= $acl->acl_check( 'action', 'edit', 'users', $my->usertype, 'section', $sectionid );
			}
		}
		if(!$access->canPublish && $_JACL->enable_jaclplus) $access->canPublish 	= $acl->acl_check( 'action', 'publish', 'users', $my->usertype, 'section', $sectionid );
		$cid = intval( mosGetParam( $_REQUEST, 'cid', 0 ) );
		if($cid && $_JACL->enable_jaclplus) {
			if(!$access->canEdit) {
				$access->canEdit = $acl->acl_check( 'action', 'add', 'users', $my->usertype, 'category', $cid );
				if(!$access->canEdit && !$_JACL->limit_edit) $access->canEdit 	= $acl->acl_check( 'action', 'edit', 'users', $my->usertype, 'category', $cid );
				if($access->canEdit || $access->canEditOwn) $from_cat = true;
			}
			if(!$access->canPublish) {
				$access->canPublish = $acl->acl_check( 'action', 'publish', 'users', $my->usertype, 'category', $cid );
				if(!$access->canPublish) $access->canPublish = $acl->acl_check( 'action', 'publish', 'users', $my->usertype, 'content', 'own' );
			}
		}
		// new record
		...
		if(!$access->canEdit && $access->canEditOwn && $_JACL->enable_jaclplus && $_JACL->limit_editown) {
			mosNotAuth();
			return;
		}
	...
	if($cid==0 && $sectionid==0) {
		$lists['catid'] 		= '';
	} else if($from_cat) {
		$lists['catid'] 		= mosAdminMenus::ComponentCategory( 'catid', $sectionid, $cid, NULL, "ordering", 1, 0, true );
	} else {
...
function saveContent( &$access, $task ) {
	global $database, $mainframe, $my, $acl, $_JACL;
	...
	if ( $isNew ) {
		if(!$access->canEdit && $_JACL->enable_jaclplus) {
			$access->canEdit = $acl->acl_check( 'action', 'add', 'users', $my->usertype, 'content', 'all' );
			if(!$access->canEdit) {
				$access->canEdit 	= $acl->acl_check( 'action', 'add', 'users', $my->usertype, 'section', $row->sectionid );
				if(!$access->canEdit && !$_JACL->limit_edit) $access->canEdit 	= $acl->acl_check( 'action', 'edit', 'users', $my->usertype, 'section', $row->sectionid );
			}
		}
		if(!$access->canPublish && $_JACL->enable_jaclplus) $access->canPublish = $acl->acl_check( 'action', 'publish', 'users', $my->usertype, 'section', $row->sectionid );
		if($row->catid && $_JACL->enable_jaclplus){
			if(!$access->canEdit) {
				$access->canEdit = $acl->acl_check( 'action', 'add', 'users', $my->usertype, 'category', $row->catid );
				if(!$access->canEdit && !$_JACL->limit_edit) $access->canEdit 	= $acl->acl_check( 'action', 'edit', 'users', $my->usertype, 'category', $row->catid );
			}
			if(!$access->canPublish) {
				$access->canPublish = $acl->acl_check( 'action', 'publish', 'users', $my->usertype, 'category', $row->catid );
				if(!$access->canPublish) $access->canPublish = $acl->acl_check( 'action', 'publish', 'users', $my->usertype, 'content', 'own' );
	...
	if (!$_JACL->enable_jaclplus || ($_JACL->publish_frontpage && $_JACL->enable_jaclplus) ) {
	...
	$msg = $access->canPublish ? _THANK_SUB_PUB: $msg;
	switch ( $task ) {
		case 'apply':
...
function cancelContent( &$access ) {
	global $database, $my, $task, $acl, $_JACL;
...
function emailContentForm( $uid, $gid ) {
	global $database, $mosConfig_hideEmail, $my;
...
function _where( $type=1, &$access, &$noauth, $gid, $id, $now=NULL, $year=NULL, $month=NULL, $params=NULL ) {
	...
	if ( $noauth ) {
		if(isset( $my->jaclplus )) {
			$where[] = "a.access IN ( $my->jaclplus )";
			$where[] = "s.access IN ( $my->jaclplus )";
			$where[] = "cc.access IN ( $my->jaclplus )";
		} else {
...
*/
function Access( &$row  ) {
	global $database, $my, $_JACL;
		
	$where = "";
	if ($_JACL->enable_jaclplus) {
		switch ($_JACL->publish_alstype) {
			case "1" :
				$where = (( $my->gid==25 ) ? "" : "\n WHERE id IN ($my->jaclplus)");
				break;
			case "2" :
				$where = (( $my->gid==25 ) ? "" : "\n WHERE id NOT IN (0,1,2)");
				break;
			case "3" :
				$where = (( $my->gid==25 ) ? "" : "\n WHERE id IN ($my->jaclplus) AND id NOT IN (0,1,2)");
				break;
			case "4" :
				$where = (( $my->gid==25 ) ? "" : "\n WHERE id IN ($_JACL->publish_jaclplus)");
				break;
			case "0" :
			default :
				$where = "";
	...
	$access = mosHTML::selectList( $groups, 'access', 'class="inputbox" size="3"', 'value', 'text', (int) $row->access );

	return $access;
}

function extraCondition( $type='0', $alias='a' ) {
	global $database, $my, $_JACL;
	
	$condition = array("","");
	if($_JACL->enable_jaclplus && $my->id) {
		$editable->category = array();
		$editable->content = array();
		$editable->contentown = false;	
	
		$query = "SELECT axo_section, axo_value FROM #__jaclplus"
		." WHERE aco_value = 'edit' AND enable = '1' AND aro_value = " . $database->Quote( $my->usertype )
		." AND (".( $type ? "axo_section = 'category' OR axo_section = 'content'" : "axo_section = 'content'" ).")"
		." ORDER BY axo_section, axo_value";
		$database->setQuery( $query );
		$rows = $database->loadRowList();
		foreach($rows as $row) {
			if($row[0]=='content') {
				if(is_numeric($row[1])){
					$editable->content[] = $row[1];
				} else {
					if($row[1]=='own') $editable->contentown = true;
				}
			}
			if($row[0]=='category') {
				if(is_numeric($row[1])){
					$editable->category[] = $row[1];
				} 
			}
		}
		if($type){
			if(!empty($editable->category)) $condition[0] .= " OR ".$alias.".catid IN (".implode(",", $editable->category).") ";
			if(!empty($editable->category)) $condition[1] .= " OR a.id IN (".implode(",", $editable->category).") ";
		}
		if(!empty($editable->content)) $condition[0] .= " OR ".$alias.".id IN (".implode(",", $editable->content).") ";
		if($editable->contentown) $condition[0] .= " OR ".$alias.".created_by = '". (int) $my->id."' ";
	}
	return $condition;
}
?>