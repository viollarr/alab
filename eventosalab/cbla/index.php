<?php
	...
	if(empty($_SESSION["id_evento"])) header("Location: http://www.alab.org.br/eventos/ix-cbla");
	...
	if ($_POST["insert"] == "true"){
		...
		/* 
		* No dia 02/06/2011, a Paula, então presidente da ALAB, enviou um email para o Daniel Costa (daniel@imaginatto.com.br) solicitando que as inscrições 
		* fossem reabertas, limitando, porém, o novo número de inscrições em mais 200. 
		* As novas inscrições seriam apenas para a modalidade de particpação SEM APRESENTAÇÃO DE TRABALHOS.
		* Por isso, a linha acima foi comentada e incluída a linha abaixo.
		* Observe também no formulário que o campo correspondente está desabilitado.
		*/
		$modalidade_participacao="0";
		//print gettype($modalidade_participacao);
		
		//$associado_alab=trim($_POST['associado_alab']);
		/* 
		* No dia 06/06/2011, a Paula, então presidente da ALAB, enviou um email para o Daniel Costa (daniel@imaginatto.com.br) 
		* solicitando que as inscrições fossem reabertas, limitando, porém, o novo número de inscrições em mais 200. 
		* As novas inscrições seriam apenas para a modalidade de particpação SEM APRESENTAÇÃO DE TRABALHOS, ou seja, como ouvinte.
		* Por isso, a linha acima foi comentada e incluída a linha abaixo.
		* Neste email ela pediu para esconder a modalidade_participacao e associado_alab.
		*/
		$associado_alab="0";
		...
		*/
		...
		if(sizeof($error)==0){
			...
			//print "<br />inserido: ".$registro_inserido;
...
<div id="tudo">
  ...
  <div id="centro">
     ...
     <?
		...
		* No dia 02/06/2011, a Paula, então presidente da ALAB, enviou um email para o Daniel Costa (daniel@imaginatto.com.br) solicitando que as inscrições 
		* fossem reabertas, limitando, porém, o novo número de inscrições em mais 200. 
		* Como até a ocasião haviam 1287 inscrições , a condição abaixo [if ($registro>=1487)] testa se foi alcançado 
		* o limite de 1487 participantes, ou seja: 1287 + 200. 
		*/
		if ($registro>=1482){
	?>     
       <p><span class="txt_titulo_destaque">Este evento j&aacute; atingiu o seu limite de participantes</span><br />
	...
	<? }else{ /**/ ?>
	  ...
	  ?>
      <p>&nbsp;</p>
       <form id="form1" name="form1" method="post" action="http://www.alab.org.br/eventosalab/cbla/index.php?id=<?php echo $_SESSION["id_evento"];?>">
...
<?php /* ?>
           ...
           <tr>
             ...
             <td><span class="txt_resposta">
               <input type="radio" name="modalidade_participacao" id="modalidade_participacao" value="1" <?php if(sizeof($error)!= 0){ if($modalidade_participacao==1){ print "checked"; }}?> disabled="disabled" />
               Com apresenta&ccedil;&atilde;o de trabalho<br />
				<input type="radio" name="modalidade_participacao" id="modalidade_participacao" value="0" <?php if(sizeof($error)!= 0){ if($modalidade_participacao==0){ print "checked"; }}?> disabled="disabled" checked="checked" />
