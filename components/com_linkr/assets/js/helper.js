...
var LinkrAPI	= new Class({
	...
	initialize : function(l, i, n, k, r)
	{
		...
		} else if ($type(_pw.tinyMCE) == 'object') {
			this.editorPlugin	= 'TinyMCE'+ _pw.tinyMCE.majorVersion;
		...
		switch(this.editorPlugin)
		{
			...
			case 'FCK2.6':
				...
				//var _n	= _pw.FCKeditorAPI.GetInstance(this.editor).Selection.GetSelectedNode();
	...
	UTF8 :
	{
		...
		Encode : function(str) {
			return this.isEncoded(str) ? str : this.encodeString(str);
		},
		Decode : function(str) {
			return this.isEncoded(str) ? this.decodeString(str) : str;
		},
		
		encodeString : function(str) {
			return (encodeURIComponent) ? unescape( encodeURIComponent( str ) ) : escape(str);
		},
		decodeString : function(str) {
			return (decodeURIComponent) ? decodeURIComponent( escape( str ) ) : unescape(str);
		}
	},
	...
	Base64 :
	{
		...
		Encode : function(str)
		{
			...
			while (i < str.length)
			{
				...
				output	+= Linkr.Base64._k.charAt(e1);
				output	+= Linkr.Base64._k.charAt(e2);
				output	+= Linkr.Base64._k.charAt(e3);
				output	+= Linkr.Base64._k.charAt(e4);
			}
		...
		Decode : function(str)
		{
			...
			while (i < str.length)
			{
				e1	= Linkr.Base64._k.indexOf(str.charAt(i++));
				e2	= Linkr.Base64._k.indexOf(str.charAt(i++));
				e3	= Linkr.Base64._k.indexOf(str.charAt(i++));
				e4	= Linkr.Base64._k.indexOf(str.charAt(i++));
			...
			return Linkr.UTF8.Decode(output);
		}
	},
	...
	urlencode : function(str)
	{
		...
		str	= (encodeURIComponent) ? encodeURIComponent(str) : escape(str);
		...
		return str.replace(/(\%([a-z0-9]{2}))/g, function(full, m1, m2) {
	...
	urldecode : function(str)
	{
		...
		return (decodeURIComponent) ? decodeURIComponent(str) : unescape(str);
	},
	...
	setCookie : function(n, v, o)
	{
		...
		// Set new values
		var old	= Linkr.getCookie(n, o, true).extend(v);
	...
	link : function(u, te)
	{
		...
		// Link text
		if	((te == this.imgAnchor) || (te == this.getSelectedText(false)))
			var t	= this.getSelectedText(true);
		else
			var t	= te;
	...
	onJsonFailure : function(r)
	{
		...
		new Element('a', {
			href : 'http://codes.statsful.com/linkr/support',
			target : '_blank',
