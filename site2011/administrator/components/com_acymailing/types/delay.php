...
class delayType{
	...
	function display($map,$value,$type = 1){
		if($type == 0){
			$this->values[] = JHTML::_('select.option', 'second',JText::_('SECONDS'));
			$this->values[] = JHTML::_('select.option', 'minute',JText::_('MINUTES'));
		}elseif($type == 1){
			$this->values[] = JHTML::_('select.option', 'minute',JText::_('MINUTES'));
		...
		$delayValue = '<input class="inputbox" onchange="updateDelay'.$this->num.'();" type="text" name="delayvalue'.$this->num.'" id="delayvalue'.$this->num.'" size="10" value="'.$return->value.'" /> ';
		$delayVar = '<input type="hidden" name="'.$map.'" id="delayvar'.$this->num.'" value="'.$value.'"/>';
		return $delayValue.JHTML::_('select.genericlist',   $this->values, 'delaytype'.$this->num, 'class="inputbox" size="1" onchange="updateDelay'.$this->num.'();"', 'value', 'text', $return->type ,'delaytype'.$this->num).$delayVar;
	}
