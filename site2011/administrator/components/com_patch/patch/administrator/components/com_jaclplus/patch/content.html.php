...
class HTML_content {
	...
	*/
	function showContentList( $title, &$items, &$access, $id=0, $sectionid=NULL, $gid, &$params, &$pageNav, $other_categories, &$lists, $order, $categories_exist ) {
		global $Itemid, $mosConfig_live_site, $_JACL;
		...
		<tr>
			...
			if ( (@$access->canAdd || $access->canEdit || $access->canEditOwn) && $categories_exist ) {
				if(!@$_JACL->enable_jaclplus || @$access->canAdd || ($access->canEdit && !@$_JACL->limit_edit && @$_JACL->enable_jaclplus) || ($access->canEditOwn && !@$_JACL->limit_editown && @$_JACL->enable_jaclplus) ) { 
				$link = sefRelToAbs( 'index.php?option=com_content&amp;task=new&amp;sectionid='. $id .'&amp;cid='. $catid .'&amp;Itemid='. $Itemid );
				?>
	...
	function showCategories( &$params, &$items, $gid, &$other_categories, $catid, $id, $Itemid ) {
		...
		foreach ( $other_categories as $row ) {
			...
			if ( $catid != $row->id ) {
				...
				<li>
					<?php
					if(class_exists('JACLPlus')) $allow = JACLPlus::InList( $row->access, $my->jaclplus );
					else $allow = $row->access <= $gid;
					//if ( $row->access <= $gid ) {
					if ( $allow ) {
						...
						?>
						<a href="<?php echo sefRelToAbs( $_JACL->noauth_link ); ?>">
							( <?php echo $_JACL->noauth_text; ?> )</a>
						<?php
						} else {
						?>
	...
	*/
	function showTable( &$params, &$items, &$gid, $catid, $id, &$pageNav, &$access, &$sectionid, &$lists, $order ) {
		global $mosConfig_live_site, $Itemid, $my, $_JACL;
			...
			<tr class="sectiontableentry<?php echo ($k+1) . $params->get( 'pageclass_sfx' ); ?>" >
				...
				if ( $params->get( 'title' ) ) {
					...
					if ( $allow ) {
						...
						echo $row->title .' : ';
					if(@$_JACL->enable_jaclplus && $my->id) {
						$link = sefRelToAbs( $_JACL->noauth_link );
						?>
						<a href="<?php echo $link; ?>">
						<?php echo $_JACL->noauth_text; ?>
						</a>
						<?php
						HTML_content::EditIcon( $row, $params, $access );
					} else {
	...
	*/
	function showLinks( &$rows, $links, $total, $i=0, $show=1, $ItemidCount=NULL ) {
		...
		for ( $z = 0; $z < $links; $z++ ) {
			...
			if(class_exists('JACLPlus')) $allow = JACLPlus::InList( $rows[$i]->access, $my->jaclplus );
			else $allow = $rows[$i]->access <= $my->gid;
			if ( $allow ) {
		...
		if ($params->get( 'readmore' ) || $params->get( 'link_titles' )) {
			if ( $params->get( 'intro_only' ) ) {
				...
				if ( $allow ) {
					...
					if(@$_JACL->enable_jaclplus && $my->id) {
					$row->link_on = sefRelToAbs( $_JACL->noauth_link );
					} else {
					...
					if ( isset($row->readmore) && @$row->readmore ) {
						if(@$_JACL->enable_jaclplus && $my->id) {
						$row->link_text	= $_JACL->noauth_text;
						} else {
	...
	*/
	function EditIcon( &$row, &$params, &$access ) {
		...
		if ( @$_JACL->enable_jaclplus && @$_JACL->edit_accessonly ) {
			if ( !JACLPlus::InList( $row->access, $my->jaclplus ) ) return;
		}
		if(!$access->canEdit && @$_JACL->enable_jaclplus && $my->id) $access->canEdit = $acl->acl_check( 'action', 'edit', 'users', $my->usertype, 'content', $row->id );
	...
	*/
	function editContent( &$row, $section, &$lists, &$images, &$access, $myid, $sectionid, $task, $Itemid ) {
		global $mosConfig_live_site, $mainframe, $_JACL;
		...
		?>
			...
			<?php if (!@$_JACL->enable_jaclplus || (@$_JACL->publish_frontpage && @$_JACL->enable_jaclplus) ) { ?>
			<tr>
				<td align="left">
				<?php echo _E_SHOW_FP; ?>
				</td>
