...
class ctrl_participante extends ctrl_generico{
	...
	function abrir_edicao(){
		...
		if(!empty($id_participante)){
			...
			$sql_user_alab = "SELECT us.id, us.name FROM jos_users us WHERE us.usertype <> 'Super Administrator' ORDER BY ltrim(rtrim(us.name)) ASC ";
			$query_user_alab = mysql_query($sql_user_alab);
			//var_dump($query_user_alab);
			$participante_novo_alab = array();
			while($result_user_alab = mysql_fetch_array($query_user_alab)){
				$sql_user_participante = "SELECT id FROM ev_participante WHERE id_evento = '".$this->id_evento."' AND id_associado_alab = '".$result_user_alab['id']."' ";
				$query_user_participante = mysql_query($sql_user_participante);
				$row_participante = mysql_num_rows($query_user_participante);
				if($row_participante == 0){
					array_push($participante_novo_alab, $result_user_alab);
				}
			}
			...
			*/
			...
			$GLOBALS["participante_novo"] = $participante_novo_alab;	
