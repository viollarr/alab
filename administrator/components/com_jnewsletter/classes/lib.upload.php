...
class upload
{
    ...
    function isMissing() {
        ...
        if (count($this->post_files) < 1) {
            return jnewsletter::printM('error' , _ACA_NO_USER_FILE);
        }
        $files = array();
        $size = 0;
        foreach ($this->post_files as $userfile => $value) {
        ...
        if ($size == 0) {
            jnewsletter::printM('error' ,_ACA_NO_USER_FILE);
        }
        return false;
    }
...
class Upload_File
{
    ...
    function moveTo($dir_dest, $overwrite = true)
    {
        ...
        if (!$this->_evalValidExtensions()) {
            return jnewsletter::printM('error' ,_ACA_NOT_ALLOWED_EXTENSION);
        }
        ...
        if (@is_file($name_dest)) {
            if ($overwrite !== true) {
                return jnewsletter::printM('error' ,_ACA_FILE_EXISTS);
            } elseif (!is_writable($name_dest)) {
                return jnewsletter::printM('error' ,_ACA_CANNOT_OVERWRITE);
            }
        }

        if (!@move_uploaded_file($this->upload['tmp_name'], $name_dest)) {
            return jnewsletter::printM('error' ,_ACA_E_FAIL_MOVE);
        }
