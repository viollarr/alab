				...
				*/
				foreach($presencas as $presenca){
					...
					if($presenca['tipo'] == 'ouvinte'){ ?>
						...
						<span class="menuinterno"><a href="gerar_certificado_presenca.php?id_trabalho=<?php //echo $mostrar['id'];?>&categoria_carta=<?php //echo $categ_carta['categoria_carta'];?>">Baixar Certificado</a></span>
						<p>&nbsp;</p>
					<?php }//if
					...
					<?php }//if
						...
						switch($presenca['tipo']){
							case 'coordenacao_simposio': 
								$sql = "SELECT id, titulo_sessao AS titulo FROM ev_simposio WHERE id_evento = '".$_SESSION['id_evento']."' AND id = '".$presenca['id_trabalho']."' ";
								$titulo_categoria = 'Coordenação de Simpósio';
								break;
								
							case 'trabalho_em_simposio': 
								$sql = "SELECT id, titulo FROM ev_resumo 
									WHERE id_evento = '".$_SESSION['id_evento']."' AND id = '".$presenca['id_trabalho']."' AND id_simposio > 0 AND id_tipo_trabalho = 1";
								$titulo_categoria = 'Trabalho em Simpósio';
								break;
