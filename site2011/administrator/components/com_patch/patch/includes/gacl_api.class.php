...
class gacl_api extends gacl {
	...
	function get_group_data($group_id, $group_type = 'ARO') {
		...
		$this->db->setQuery( 'SELECT group_id, parent_id, name, lft, rgt, jaclplus FROM '. $table .' WHERE group_id='. (int) $group_id );
	...
	\*======================================================================*/
	/* Hack for integration with JACLPlus Component */
	function add_group($name, $parent_id=0, $group_type='ARO', $jaclplus='0') {
		...
		if (strlen($jaclplus)<1) $jaclplus='0';
		...
		if ($parent_id == 0) {
			...
			if (!is_array( $rs )) {
				...
				// <jaclplus> $this->db->RollBackTrans();
				return FALSE;
			}
		...
		$this->db->setQuery( 'INSERT INTO '. $table .' (group_id,parent_id,name,lft,rgt,jaclplus) VALUES ('. (int) $insert_id .','. (int) $parent_id .',\''. $this->db->getEscaped($name) .'\','. (int) $parent_rgt .','. (int) ($parent_rgt + 1) .','. $this->db->Quote($jaclplus) .')' );
		$rs = $this->db->query();
	...
	\*======================================================================*/
	/* Hack for integration with JACLPlus Component */
	function edit_group($group_id, $name=NULL, $parent_id=NULL, $group_type='ARO', $jaclplus=NULL) {
		$this->debug_text("edit_group(): ID: $group_id Name: $name ALs: $jaclplus Parent ID: $parent_id Group Type: $group_type");
		...
		if (!empty($parent_id)) {
			...
			if (!$this->get_group_name($parent_id, $group_type)) {
		...
		// update als if it is specified.
		if (strlen($jaclplus)>0) {
			$set[] = 'jaclplus='. $this->db->Quote($jaclplus);
		}
		...
		$this->db->setQuery( 'UPDATE '. $table .' SET '. implode(',', $set) .' WHERE group_id='. (int) $group_id );
		$rs = $this->db->query();
		...
		if (!empty($parent_id)) {
			if (!$this->_rebuild_tree($table, 17)) { // 17 - ROOT group_id
				//$this->db->RollbackTrans();
				return FALSE;
			}
		}
	...
	/*======================================================================*\
		...
		Purpose:	Utility recursive function called by rebuild_tree()
	\*======================================================================*/
	function _rebuild_tree($table, $group_id, $left = 1) {
		...
		$this->db->setQuery( "SELECT group_id FROM ". $table ." WHERE parent_id=". (int) $group_id );
		$rows = $this->db->loadRowList();
		if ($this->db->getErrorNum()) {
		...
		$this->db->setQuery( "UPDATE ". $table ." SET lft=". (int) $left .", rgt=". (int) $right ." WHERE group_id=". (int) $group_id);
		$rs = $this->db->query();
...
//			SELECT		g.group_id,o.'. $group_type .'_id,(gm.group_id IS NOT NULL) AS member
	...
	function _getBelow( $table, $fields, $groupby=null, $root_id=null, $root_name=null, $inclusive=true, $limitstart=0, $limit=0 ) {
		global $database;
