...
print "<hr>relatorio_posteres.php:<pre>";
	print_r($comunicacoes_individuais);
print "</pre>";
/**/
?>
...
Relat&oacute;rio de Comunica&ccedil;&otilde;es Individuais
<br /><br />

<div style="margin-bottom:20px; margin-top:-5px; border:1px solid #0099cc; padding:10px; background:#fcfcfc">
    <h3>Quantidade de Comunica&ccedil;&otilde;es Individuais inscritas (<?php echo count($comunicacoes_individuais); ?>)</h3>
    <br />
    <?php
    ...
    <table border="1" width="100%" cellpascing="1" cellpadding="0">
        ...
        if( empty($_GET['id_linha_tematica']) ){ 
            ...
            for ($i=0; $i < $rows; $i++){
                ...
                for($j=0; $j < $columns; $j++){
                    echo "<td>";
                        $item = "<a href=\"controle.php?ctrl=relatorio&acao=listar_comunicacoes_individuais&id_linha_tematica=".$linhas_tematicas[$k]['id']."\">- ".$linhas_tematicas[$k]['titulo'];
            ...
            foreach($linhas_tematicas as $linha_tematica){
                if($linha_tematica['id'] == $_GET['id_linha_tematica']){
                    echo "<tr>";
                        echo "<td>";
                            $item = "<a href=\"controle.php?ctrl=relatorio&acao=listar_comunicacoes_individuais&id_linha_tematica=".$linha_tematica['id']."\">".$linha_tematica['titulo'];
...
<?php 
if(!empty($comunicacoes_individuais)){
	foreach($linhas_tematicas as $linha_tematica){
		foreach($comunicacoes_individuais as $comunicacao_individual){ 
			if($linha_tematica["id"] == $comunicacao_individual["id_linha_tematica"]){ 
				?>
                ...
                <?php if($comunicacao_individual["aceito"]){ ?>
                ...
                <div style="text-transform:uppercase; font-weight:bold; margin-bottom:20px;"><?php echo $comunicacao_individual["titulo"]; ?></div>
                <div style="margin-bottom:10px;">
                    <?php 
                        if(!empty($participantes)){
                            foreach($participantes as $participante){
                                if($participante["id"] == $comunicacao_individual["autor"]) {
                                    echo $participante["nome"];
                                    echo "<br />".$participante["instituicao"];
                                }//if
                            }//foreach
                            foreach($participantes as $participante){
                                if($participante["id"] == $comunicacao_individual["co_autor"]) {
                ...
                <div style="margin-bottom:10px; text-align:justify;"><strong>Resumo:</strong> <?php echo $comunicacao_individual["resumo"]; ?></div>
                <div style="margin-bottom:30px;"><strong>Palavras-chave:</strong> <?php echo $comunicacao_individual["palavras_chave"]; ?></div>
			<?php }//if
		}//foreach 
	}//foreach
} //if
