         ...
         <?php 
     				while($mostrar_trabalho=mysql_fetch_array($qr_trabalho)){ 
						if ($asssociado_alab==1){ 
							//if ( ($mostrar_trabalho['id']==1) and ($id_tipo_participante!=4) and ($id_tipo_participante!=5) and ($id_tipo_participante!=6)) { //idtrabalho 1 -> simposio | idparticipante 6 -> aluno graduação
								if ( $mostrar_trabalho['id']==1) { //idtrabalho 1 -> simposio | idparticipante 6 -> aluno graduação							
								if(($id_formacao!=1) and ($id_formacao!=3)) {
									continue;
								}
			 ?>
