...
class plgAcymailingShare extends JPlugin
{
	function plgAcymailingShare(&$subject, $config){
	 ...
	 function acymailingtagshare_show(){
		...
		$networks['hyves'] = JText::sprintf('SOCIAL_SHARE','Hyves');
		$networks['google'] = JText::sprintf('SOCIAL_SHARE','Google+');
		$k = 0;
		...
		foreach($results as $var => $allresults){
			foreach($allresults[0] as $i => $tagname){
				...
				if($tag->network == 'facebook'){
					...
					$tags[$tagname] = '<a target="_blank" href="http://www.hyves-share.nl/button/respect/?hc_hint=1&url='.urlencode($archiveLink).'&title='.urlencode($email->subject).'" title="'.JText::sprintf('SOCIAL_SHARE','Hyves').'"><img src="'.ACYMAILING_LIVE.$this->params->get('picturehyves','media/com_acymailing/images/hyvesshare.png').'" /></a>';
				}elseif($tag->network == 'google'){
					$tags[$tagname] = '<a target="_blank" href="https://plusone.google.com/_/+1/confirm?url='.urlencode($archiveLink).'&title='.urlencode($email->subject).'" title="'.JText::sprintf('SOCIAL_SHARE','Google+').'"><img src="'.ACYMAILING_LIVE.$this->params->get('picturegoogleplus','media/com_acymailing/images/google_plusshare.png').'" /></a>';
				}
			}
		}
