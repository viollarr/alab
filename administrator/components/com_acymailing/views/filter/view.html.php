...
class FilterViewFilter extends JView
{
	...
	function form(){
		...
		$js .= "function countresults(num){
					...
					try{
						new Ajax('index.php?'+$('adminForm').toQueryString()+'&option=com_acymailing&tmpl=component&ctrl=filter&task=countresults&num='+num,{ method: 'post', update: document.getElementById('countresult_'+num)}).request();
					}catch(err){
						new Request({
						method: 'post',
						url: 'index.php?'+$('adminForm').toQueryString()+'&option=com_acymailing&tmpl=component&ctrl=filter&task=countresults&num='+num,
