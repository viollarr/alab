...
class toggleHelper{
		...
		if($params->mode == 'pictures'){
			...
			if(!$pictureincluded){
				...
				$js = "function joomTogglePicture(id,newvalue,table){
					...
					new Ajax('index.php?option=com_acymailing&tmpl=component&gtask=toggle&task='+id+'&value='+newvalue+'&table='+table,{ method: 'get', update: $(id), onComplete: function() {	window.document.getElementById(id).className = 'loading'; }}).request();
				}";
			...
			if(!$classincluded){
				...
				$js = "function joomToggleClass(id,newvalue,table,extra){
					...
					new Ajax('index.php?option=com_acymailing&tmpl=component&gtask=toggle&task='+id+'&value='+newvalue+'&table='+table+'&extra[color]='+extra,{ method: 'get', update: $(id), onComplete: function() {	window.document.getElementById(id).className = 'loading'; }}).request();
				}";
				$doc =& JFactory::getDocument();
				$doc->addScriptDeclaration( $js );
			}
	...
	function delete($lineId,$elementids,$table,$confirm = false){
		...
		if(!$deleteJS){
			...
			$js = "function joomDelete(lineid,elementids,table,reqconfirm){
				...
				new Ajax('index.php?option=com_acymailing&tmpl=component&gtask=".$this->gtask.$this->extra."&task=delete&value='+elementids+'&table='+table, { method: 'get', onComplete: function() {window.document.getElementById(lineid).style.display = 'none';}}).request();
			}";
			$doc =& JFactory::getDocument();
			$doc->addScriptDeclaration( $js );
		}
		return '<a href="javascript:void(0);" onclick="joomDelete(\''.$lineId.'\',\''.$elementids.'\',\''.$table.'\','. ($confirm ? 'true' : 'false').')"><img src="'.ACYMAILING_IMAGES.'delete.png"/></a>';
	}
}
