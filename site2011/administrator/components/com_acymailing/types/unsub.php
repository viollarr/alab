...
class unsubType{
	function unsubType(){
		...
		$js = "function changeMessage(idField,value){
			...
			if(value>0){
				window.document.getElementById(linkEdit).href = 'index.php?option=com_acymailing&tmpl=component&gtask=email&task=edit&mailid='+value;
	...
	function display($value){
		...
		$linkAdd = 'index.php?option=com_acymailing&amp;tmpl=component&amp;gtask=email&amp;task=add&amp;type=unsub';
		$style = empty($value) ? 'style="display:none"' : '';
		$text = ' <a '.$style.' class="modal" id="unsub_edit" title="'.JText::_('EDIT_EMAIL').'"  href="'.$linkEdit.'" rel="{handler: \'iframe\', size: {x: 800, y: 500}}"><img src="../images/M_images/edit.png" alt="'.JText::_('EDIT_EMAIL',true).'"/></a>';
		$text .= ' <a class="modal" id="unsub_add" title="'.JText::_('CREATE_EMAIL').'"  href="'.$linkAdd.'" rel="{handler: \'iframe\', size: {x: 800, y: 500}}"><img src="../images/M_images/new.png" alt="'.JText::_('CREATE_EMAIL',true).'"/></a>';
