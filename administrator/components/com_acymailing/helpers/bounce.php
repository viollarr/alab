...
class bounceHelper{
  ...
  function handleMessages(){
    ...
    while(($msgNB>0) && ($this->_message = imap_headerinfo($this->mailbox,$msgNB))){
      ...
      if(empty($resultsSubid[1])){
        ...
        $removeEmails = '#'.str_replace(array('%'),array('@'),$this->config->get('bounce_username')).'|'.$this->config->get('from_email').'|'.$this->config->get('reply_email');
        $other = $this->config->get('bounce_email');
        if(!empty($other)) $removeEmails .= '|'.$this->config->get('bounce_email');
        $removeEmails .='#i';
        if(!empty($results[0])){
		...
		if(!empty($this->_message->subemail)) $this->_message->subid = $this->subClass->subid($this->_message->subemail);
      }else{
      ...
      $isBounce = $this->_handleAction('bounce');
      if(!$isBounce){
        $actiondetected = $this->_handleActions();
        if(!$actiondetected) $this->_handleAction('end');
      }
    }
    ob_start();
  }
  ...
  function _handleAction($num){
    ...
    if(!empty($this->_message->subid)){
		...
		switch($subaction){
			...
			case 'delete' :
				...
				$this->subClass->delete($this->_message->subid);
	      }
    }else{
    ...
    if($num == 'bounce'){
    	if(!empty($this->_message->subid) AND !empty($this->_message->mailid)){
    		$this->db->setQuery('UPDATE '.acymailing::table('userstats').' SET `bounce` = `bounce` + 1 WHERE `subid` = '.(int) $this->_message->subid.' AND `mailid` = '.(int) $this->_message->mailid.' LIMIT 1');
    		$this->db->query();
    	}
    	if(!empty($this->_message->mailid)){
    		$this->db->setQuery('UPDATE '.acymailing::table('stats').' SET `bounceunique` = `bounceunique` + 1 WHERE `mailid` = '.(int) $this->_message->mailid.' LIMIT 1');
    		$this->db->query();
    	}
    }
