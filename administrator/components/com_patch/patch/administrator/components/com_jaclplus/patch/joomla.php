...
if(file_exists($mosConfig_absolute_path . '/administrator/components/com_jaclplus/jaclplus.class.php')) {
  ...
  /**
	...
	*/
	function initSession() {
		...
		if ( $sessioncookie && strlen($sessioncookie) == 32 && $sessioncookie != '-' && $session->load($sessionValueCheck) ) {
			...
			/* Hack for integration with JACLPlus Component */
			if(class_exists('JACLPlus')) $session->jaclplus	= JACLPlus::DefaultAL();
			/* End of hack for JACLPlus Component */
	...
	*/
	function initSessionAdmin($option, $task) {
		...
		/* Hack for integration with JACLPlus Component */
		if(class_exists('JACLPlus')) $my->jaclplus	= mosGetParam( $_SESSION, 'session_jaclplus', '' );
		/* End of hack for JACLPlus Component */
		...
		/* Hack for integration with JACLPlus Component */
		if(class_exists('JACLPlus')) JACLPlus::AccessChecking($my, 'backend');
		/* End of hack for JACLPlus Component */
		return $my;
	}
	...
	function login( $username=null, $passwd=null, $remember=0, $userid=NULL ) {
		...
		if (!$username || !$passwd) {
			...
			if (is_object($row)) {
				...
				if(class_exists('JACLPlus')) $session->jaclplus		= $grp->jaclplus;
				/* End of hack for JACLPlus Component */
	...
	*/
	function getUser() {
		...
		/* Hack for integration with JACLPlus Component */
		if(class_exists('JACLPlus')) {
			$jaclplus = explode( ',', $this->_session->jaclplus );
			mosArrayToInts( $jaclplus );
			$user->jaclplus		= implode( ',', $jaclplus );
		}
		/* End of hack for JACLPlus Component */
		...
		/* Hack for integration with JACLPlus Component */
		if(class_exists('JACLPlus')) JACLPlus::AccessChecking($user, 'frontend');
		/* End of hack for JACLPlus Component */
		return $user;
	}
	/**
...
function mosMenuCheck( $Itemid, $menu_option, $task, $gid ) {
	...
	if(class_exists('JACLPlus')) {
		...
		foreach ($results as $result) {
			if( !JACLPlus::InList( $result->access, $my->jaclplus ) ) {
				$access = false;
				break;
			}
		}
	...
	if(class_exists('JACLPlus')) return $access;
	else return ($access <= $gid);
}
/* End of hack for JACLPlus Component */
...
class mosMambotHandler {
	...
	*/
	function loadBotGroup( $group ) {
		...
		if (is_object( $my )) {
			$gid = $my->gid;
			if(class_exists('JACLPlus')) $jaclplus = $my->jaclplus;
		} else {
			$gid = 0;
			if(class_exists('JACLPlus')) $jaclplus = JACLPlus::DefaultAL();
		}
...
class mosAdminMenus {
	...
	*/
	function ComponentCategory( $name, $section, $active=NULL, $javascript=NULL, $order='ordering', $size=1, $sel_cat=1, $selfonly=false ) {
		global $database;
