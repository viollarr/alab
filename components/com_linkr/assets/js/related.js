...
var LinkrRelated	=
{
	...
	_psc : function(w, a)
	{
		...
		u	+= '&kw='+ encodeURIComponent(Linkr.UTF8.Encode(w));
		if (a) {
			u	+= '&title='+ encodeURIComponent(Linkr.UTF8.Encode(a.title || ''));
			u	+= '&limit='+ (a.limit || '5');
			u	+= '&exclude='+ (a.exclude || '');
		}
		...
		/*new Json.Remote(u, {
			onComplete : LinkrRelated.displayPreview.bind(this)
		}).send();*/
	},
	...
	_usc : function(w, l, t, x)
	{
		// Get request URL
		var u	= Linkr.createRequest('ra') +
					'&kw='+ encodeURIComponent((Linkr.UTF8.Encode(w))) +
					'&limit='+ l.toInt() +
					'&title='+ encodeURIComponent(Linkr.UTF8.Encode(t));
