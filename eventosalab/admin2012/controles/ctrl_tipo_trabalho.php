...
class ctrl_tipo_trabalho extends ctrl_generico{
	...
	*/
	public function trabalhos_participante(){
		...
		$resumos_autor = $resumos_co_autor = $resumos_co_autor2 = $resumos_co_autor3 = array();
		...
		else
		{
			...
			// Verifica se Co-autor: Pфster, Comunicaзгo Individual, resumo de Comunic. Coordenada e resumo de Simpуsio
			$sql = "SELECT * FROM ev_resumo WHERE co_autor2='".$id_participante."' AND id_evento = '".$this->id_evento."' ";
			$result = mysql_query($sql);
			while($linha = mysql_fetch_array($result)){
				array_push($resumos_co_autor2, $linha);			
			} //while	

			// Verifica se Co-autor: Pфster, Comunicaзгo Individual, resumo de Comunic. Coordenada e resumo de Simpуsio
			$sql = "SELECT * FROM ev_resumo WHERE co_autor3='".$id_participante."' AND id_evento = '".$this->id_evento."' ";
			$result = mysql_query($sql);
			while($linha = mysql_fetch_array($result)){
				array_push($resumos_co_autor3, $linha);			
			} //while	
			...
			foreach($resumos_co_autor2 as $resumo_co_autor2){
				array_push($resumos, $resumo_co_autor2);
			}//foreach
			foreach($resumos_co_autor3 as $resumo_co_autor3){
				array_push($resumos, $resumo_co_autor3);
			}//foreach			
		}
