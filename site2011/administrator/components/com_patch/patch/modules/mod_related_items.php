			...
			if (count($temp)) {
				foreach ($temp as $row ) {
					if(isset($my->jaclplus)){
						if (($row->cat_state == 1 || $row->cat_state == '') &&  ($row->sec_state == 1 || $row->sec_state == '') &&  (in_array( $row->cat_access, explode( ",", $my->jaclplus ) ) || $row->cat_access == '') &&  (in_array( $row->sec_access, explode( ",", $my->jaclplus ) ) || $row->sec_access == '')) {
							$related[] = $row;
						}
					} else {
