...
function showUsers( $option ) {
	...
	if( $my->gid != 23 && $my->gid != 24 && $my->gid != 25) {
		array_push($pgids, "23", "24", "25");
	}

	mosArrayToInts( $pgids );
	if (is_array( $pgids ) && count( $pgids ) > 0) {
		$where[] = '( a.gid != '  . implode( ' AND a.gid != ', $pgids ) . ' )';
	}
	...
	;
	if (is_array( $pgids ) && count( $pgids ) > 0) {
		$query .= "\n AND ( group_id != " . implode( ' AND group_id != ', $pgids ) . " )";
	}
...
function saveUser( $task ) {
	...
	if ($isNew) {
		...
		if ( $row->gid != $original->gid ) {
			...
			} else if ( $my->gid != 25 && $original->gid == 24 ) {
				// disallow change of super-Admin by non-super admin
...
function getGIDSChildren($gid) {
	...
	if( $gid != 23 && $gid != 24 && $gid != 25) {
		$where = "\n AND ( g1.group_id != 23 AND g1.group_id != 24 AND g1.group_id != 25 )";
	} else {
		$where = "\n OR g1.group_id > 30";
	}
	...
	."\n WHERE g2.group_id = " . (int) $gid . $where
	."\n ORDER BY g1.name"
	;
	$database->setQuery( $query );
...
function moveUsers( $cid, $option ) {
	...
	if ( !$msg && count( $cid ) ) {
		...
		foreach ($cid as $id) {
			...
			if ( $count <= 1 && $obj->gid == 25 ) {
			// cannot move Super Admin where it is the only one that exists
				...
				// logout user acounts active sessions
