	...
	<table class="adminlist" cellpadding="1">
		<thead>
			<tr>
				...
				<th class="title">
					<?php echo JHTML::_('grid.sort',   JText::_('USERNAME'), 'a.username',$this->pageInfo->filter->order->dir, $this->pageInfo->filter->order->value ); ?>
				</th>
				<th class="title titleid">
					<?php echo JHTML::_('grid.sort',   JText::_('ID'), 'a.id', $this->pageInfo->filter->order->dir, $this->pageInfo->filter->order->value ); ?>
				</th>
			</tr>
		</thead>
		<tfoot>
		...
		<tbody>
			<?php
				...
				for($i = 0,$a = count($this->rows);$i<$a;$i++){
					$row =& $this->rows[$i];
			?>
				<tr class="<?php echo "row$k"; ?>" style="cursor:pointer" onclick="window.top.affectUser(<?php echo $row->id?>,'<?php echo addslashes($row->name); ?>'); window.top.document.getElementById('sbox-window').close();">
					<td align="center">
