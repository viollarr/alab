...
mysql_select_db($banco); 
    ...
    $sql_busca = "SELECT name,block,endereco_res,complemento_res,bairro_res,cidade_res,estado_res,cep_res, tipo_residencia, id_cidade_res, id_estado_res  
				  ...
				  WHERE block='0' AND id<>'62' AND gid='19' AND tipo_anuidade='Aluno' ORDER BY name ASC";
...
setlocale(LC_ALL, 'pt_BR');
	...
	while($mostrar = mysql_fetch_array($query_busca)) {
		...
		$tipo_residencia=$mostrar['tipo_residencia'];
		$id_cidade_res=$mostrar['id_cidade_res'];
		$id_estado_res=$mostrar['id_estado_res'];

		if ($tipo_residencia=='1'){
			$id_cidade_res=$mostrar['id_cidade_res'];
			$id_estado_res=$mostrar['id_estado_res'];
			
			    if (!empty($id_cidade_res)){
					$sql_cidade = "SELECT nome 
								   FROM ev_cidades
								   WHERE cod_cidades='$id_cidade_res'";
					$query_cidade=mysql_query($sql_cidade, $conexao);
					$mostrar_cidade = mysql_fetch_array($query_cidade);
					$cidade = $mostrar_cidade['nome'];
				}
			    if (!empty($id_estado_res)){				
					$sql_estado = "SELECT nome 
								   FROM ev_estados
								   WHERE cod_estados='$id_estado_res'";
					$query_estado=mysql_query($sql_estado, $conexao);
					$mostrar_estado = mysql_fetch_array($query_estado);
					$estado = $mostrar_estado['nome'];	
				}
	...
	header('Content-Disposition: attachment; filename=associados-alunos-'.date('d-m-Y-His').'.doc');
