...
class SManager
{
	...
	function getSubmissions()
	{
		...
		if ($this->export)
		{
			...
			$result = mysql_query("SELECT * FROM `".$RSadapter->tbl_rsform_submissions."` s LEFT JOIN `".$RSadapter->tbl_rsform_submission_values."` sv ON s.`SubmissionId` = sv.`SubmissionId` WHERE s.`FormId` = '".$this->formId."' ".($this->rows != 0 ? "AND s.`SubmissionId` IN (".implode(',',$this->rows).")" : "")." ORDER BY `s`.`DateSubmitted` DESC");
		}
		...
		else
		{
			...
			$result = mysql_query("SELECT DISTINCT sv.`SubmissionId` FROM `".$RSadapter->tbl_rsform_submissions."` s LEFT JOIN `".$RSadapter->tbl_rsform_submission_values."` sv ON s.`SubmissionId` = sv.`SubmissionId` WHERE s.`FormId` = '".$this->formId."' ".($this->order > 1 ? " AND sv.`FieldName`='".RScleanVar($this->headers[$this->order])."'" : "")." ".$filter." ORDER BY ".$order." ".$this->direction." LIMIT ".$this->current*$this->limit.",".$this->limit);
			...
			// Get data for the submission ids
			$result = mysql_query("SELECT * FROM `".$RSadapter->tbl_rsform_submissions."` s LEFT JOIN `".$RSadapter->tbl_rsform_submission_values."` sv ON s.`SubmissionId` = sv.`SubmissionId` WHERE s.`FormId` = '".$this->formId."' AND s.`SubmissionId` IN (".implode(',',$submission_ids).") ORDER BY ".$order." ".$this->direction);
	...
	function createExportFile()
	{
		...
		foreach ($submissions as $submissionId => $submission)
		{
			foreach ($this->exportOrder as $orderId => $header)
				echo $this->exportFieldEnclosure.(isset($submission['SubmissionValues'][$header]) ? str_replace(array("\r\n",$this->exportFieldEnclosure), array("\n",$this->exportFieldEnclosure.$this->exportFieldEnclosure), $submission['SubmissionValues'][$header]['Value']) : (isset($submission[$header]) ? $submission[$header] : '')).$this->exportFieldEnclosure.($header != end($this->exportOrder) ? $this->exportDelimiter : "");
			echo "\n";
		}
	...
	function pager($current=1)
	{
		...
		// Get submission ids first
		$submission_ids = array();
		$total = @mysql_result(mysql_query("SELECT COUNT(DISTINCT(sv.`SubmissionId`)) FROM `".$RSadapter->tbl_rsform_submissions."` s LEFT JOIN `".$RSadapter->tbl_rsform_submission_values."` sv ON s.`SubmissionId` = sv.`SubmissionId` WHERE s.`FormId` = '".$this->formId."' ".($this->order > 1 ? " AND sv.`FieldName`='".RScleanVar($this->headers[$this->order])."'" : "")." ".$filter),0);
