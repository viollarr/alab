...
class ctrl_avaliacao extends ctrl_generico{
	...
	*/
	function alocar_pareceristas(){
		...
		$GLOBALS["avaliadores"]      = $this->select("SELECT ev.id, us.name, us.email FROM ev_participante ev, jos_users us WHERE ev.id_evento = '".$this->id_evento."' AND ev.id_formacao = 1 AND ev.id_associado_alab = us.id ORDER BY us.name ASC");
	...
	*/
	function relacionar_avaliador_trabalhos(){
		...
		$GLOBALS["avaliadores"] = 
			$this->select("SELECT ev.id, us.name, us.email FROM ev_participante ev, jos_users us WHERE ev.id_evento = '".$this->id_evento."' AND ev.avaliador = 'sim' AND ev.id_associado_alab = us.id ORDER BY us.name ASC");
		
		return "relacionar_avaliador_trabalhos";
	}
		...
		*/
		...
		foreach($linhas_tematicas as $linha_tematica){
			...
			foreach($papers as $paper) {
				$paper["tipo_trabalho"] = "paper";
				array_push($trabalhos, $paper);
			}//foreach
			...
			if(is_array($avaliadores)){
				...
				foreach($avaliadores as $id_avaliador){
					...
					while($k < count($trabalhos)){
						//echo $id_avaliador." => ".$trabalhos[$k]["id"]."<br/>";
	...
	*/
	function salvar_associacao($id_avaliador, $trabalho){
		if(!empty($id_avaliador) && !empty($trabalho)){
			...
			$select_trabalhos_existe_avaliacao = $this->select("SELECT id, id_trabalho FROM ev_avaliacao WHERE id_trabalho = '".$trabalho["id"]."' AND id_evento = ".$this->id_evento."");
			
			if(!empty($select_trabalhos_existe_avaliacao)){ // se já houver ele apenas atualiza.
				foreach($select_trabalhos_existe_avaliacao as $id_existente){
					$sql = "UPDATE ev_avaliacao SET id_avaliador = '".$id_avaliador."', created = '".$created."'  WHERE id = '".$id_existente["id"]."'";
	...
	function determinar_avaliadores(){
		$GLOBALS["doutores"] = $this->select("SELECT ev.id, us.name, ev.avaliador, us.email FROM ev_participante ev, jos_users us WHERE ev.id_evento = '".$this->id_evento."' AND us.titulacao_academica = 'Doutor' AND ev.id_associado_alab = us.id ORDER BY us.name ASC");
	...
	function salvar_avaliadores(){
		$doutores = $this->select("SELECT ev.id, us.name, ev.avaliador, us.email FROM ev_participante ev, jos_users us WHERE ev.id_evento = '".$this->id_evento."' AND us.titulacao_academica = 'Doutor' AND ev.id_associado_alab = us.id ORDER BY us.name ASC");
