...
if(html){if(typeof o.html=='undefined'){o.html=[];}
o.html.push(html);}
...
return;type=matches[1].toLowerCase();data=JSON.parse(matches[2]);w=data.width;h=data.height;name='object';}else{name=n.name;var style=Styles.parse(n.attr('style'));var w=n.attr('width')||style.width||'';var h=n.attr('height')||style.height||'';var type=n.attr('type');if(name=='embed'&&type=='application/x-shockwave-flash'){name='object';each(['bgcolor','flashvars','wmode','allowfullscreen','allowscriptaccess','quality'],function(k){var v=n.attr(k);if(v){if(k=='flashvars'){try{v=encodeURIComponent(v);}catch(e){}}
data[k]=v;}});}
data=this.createTemplate(n);each(['audio','embed','object','video','iframe'],function(el){each(n.getAll(el),function(node){node.remove();});});if(n.attr('classid')){classid=n.attr('classid').toUpperCase();}
if(name=='object'){if(!data.data){var param=data.param;if(param){data.data=param.src||param.url||param.movie||param.source;}}}else{if(!data.src){if(data.source){data.src=data.source[0].src;}}}
var lookup=this.lookup[classid]||this.lookup[type]||this.lookup[name]||this.lookup['flash'];type=lookup.name||'';var style=Styles.parse(n.attr('style'));each(['bgcolor','align','border','vspace','hspace'],function(na){var v=n.attr(na);if(v){switch(na){case'bgcolor':style['background-color']=v;break;case'align':if(/^(left|right)$/.test(v)){style['float']=v;}else{style['vertical-align']=v;}
...
name=name.toLowerCase();if(type){type=type.toLowerCase();}
classes.push('mceItem'+ucfirst(name));if(type&&name!==type){classes.push('mceItem'+ucfirst(type));}
...
img.attr({src:this.url+'/img/trans.gif',width:w,height:h,'class':classes.join(' '),'data-mce-json':JSON.serialize(o)});},serializeAttributes:function(n){var self=this,dom=this.editor.dom,attribs={},ti='';if(n!='iframe'||n!='param'){var type=n.attr('type'),src=n.attr('src')||n.attr('data');if(!type&&src){var ext;if(/\.([a-z0-9]{2,4})/.test(src)){ext=/\.([a-z0-9]{2,4})/.exec(src);ext=ext[1]||'';}
...
attribs[k]=v;}else{for(k in n.attributes.map){v=n.attributes.map[k];switch(k){case'poster':case'src':case'data':attribs[k]=self.convertUrl(v);break;case'autoplay':case'controls':case'loop':attribs[k]=k;break;case'allowfullscreen':attribs[k]=(v=='')?true:v;break;case'type':attribs[k]=v.replace(/"/g,"'");break;default:attribs[k]=v;break;}}}
...
return type;},restoreElement:function(n,args){var self=this,ed=this.editor,dom=ed.dom,cl,props;var data=JSON.parse(n.attr('data-mce-json'));var name=this.getNodeName(n.attr('class'));var parent=new Node(name,1);var root=data[name];var src=root.src||root.data||'';var params=root.param||'';each(['width','height'],function(k){v=n.attr(k);if(v&&name!='audio'){if(!root[k]||root[k]!=v){root[k]=v;}}
...
params=params||{};delete root.src;if(strict){root.data=src;extend(params,{'movie':src});delete params.src;delete root.embed;delete root.classid;delete root.codebase;}else{if(!root.embed){root.embed={width:root.width,height:root.height,src:src,type:root.type||this.getMimeType(n.attr('class'))||this.getMimeType(src)};}
var lookup=this.lookup[root.type]||this.lookup[name]||this.lookup['flash'];if(!root.classid){root.classid=lookup.classid;}
if(!root.codebase){root.codebase=lookup.codebase;}
for(k in params){if(/^(movie|source|url)$/.test(k)){root.embed.src=params[k];}else{root.embed[k]=params[k];}}
var k='src';if(/mceItemWindowsMedia/.test(n.attr('class'))){k='url';}
if(/mceItemSilverLight/.test(n.attr('class'))){k='source';}
params[k]=src;var props=this.lookup[name];extend(root,props);delete root.data;delete root.type;}}else{if(root.src&&root.source){delete root.src;}}
